...
| 卡 | 失败的脚本 | 原因 | 归属 |
|---|---|---|---|
| 银麒赎世 | 银麒系统面板 | `refused document.readyState`(虚拟 document 拒绝良性读) | 7b 在改,非存储 |
...
| 锚点 | 判据 | **也会绿** |
|---|---|---|
| `#send_textarea` + `#send_but` | 写值 → 点击 → **消息真的发出**,且出一条 note 说明是卡代发的 | 提供一个**接受写入但不发送**的假节点:卡的代码路径全绿、用户什么也没看到 |
