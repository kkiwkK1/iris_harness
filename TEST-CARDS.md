...
- **状态**:**两半都已定稿。** 写路径(真模型两次生成,爱衣;写方=宿主 `recordVariables`,
...
**语料里 5 条 ≥80 消息的聊天全是这个形状。** 所以这条格子要用**重建的表**(49 已做:剪 311/338),
不能直接用磁盘上的语料。

#### ⚠ 验收前提:真宿主上默认**关**,必须在显式开启的 profile 上做
...
> —— 又一次「印证只在分叉层有效」(§十九)。
>
> **所以清理规则的验收要在_显式开启_的 profile 上做**;在默认 profile 上跑会得到
> 「什么都没清」,而那**既正确又无判别力**(和"已清过的语料"是同一种空)。

**一处与上游的偏离,值得单独确认是不是有意的:**
我先前三路交叉确认过**上游 MVU 的自动清理默认是_开_的**(磁盘分布 / `settings.json` /
zod 默认值三者一致,§七之二 开头)。**我们默认关。**
→ 后果:**导入的旧聊天是上游已经清过的**(所以它们幸存点在 0/50/…),
而**我们此后不会继续清**,除非显式开启。**这是"兼容地板"上的一处实质差异,
按两本账应当记进偏离那本**;是否有意由你裁,我只报事实。
