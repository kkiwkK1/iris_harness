...
import { stringHash as core } from '@iris/compat-tavernhelper-core'

import { stringHash as local } from '../src/activate.ts'
 ...
 * package checks its own re-export against core; transitivity does the rest.
 */

test('the exported hash is the one in core', () => {
  assert.equal(local, core, 'this package grew its own copy of the hash')
})
