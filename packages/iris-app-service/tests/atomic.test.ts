...
test('a replace the platform refuses for a moment is waited out, not failed', async (t) => {
  ...
  // **Deterministic, and it is the Windows case that made the retry necessary.**
  // Measured on this machine: while any handle is open on the target, `rename`
  // onto it comes back `EPERM` — a reader (`chat.search` scanning the profile,
  // a backup being taken, an antivirus opening the new file) is enough. Without
  // the retry the save would simply fail; with it, it waits the reader out.
  //
  // On a POSIX host `rename` over an open file succeeds immediately and this
  // test passes without exercising the retry at all. That is fine and is said
  // out loud: the gate for this branch runs on Windows, where it has teeth.
  const held = await open(path, 'r')
  let closed = false
  const writing = atomicWriteFile(path, 'REPLACED\n')
  setTimeout(() => { closed = true; void held.close() }, 80)

  await writing
  assert.equal(closed, true, 'the write finished before the handle was released')
