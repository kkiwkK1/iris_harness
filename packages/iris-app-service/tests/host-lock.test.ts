 ...
 *
 * Every temporary directory here comes from `tempDir`. Nothing in this file may
...
/** A fresh, empty data directory nobody else has, removed when the test ends. */
async function dataDir(t: TestContext): Promise<string> {
  return await tempDir(t, 'iris-hostlock-')
}
...
test('a fresh directory is taken, and the file says who took it', async (t: TestContext) => {
...
test('a second host on the same directory is refused, and the sentence says where, who, which port and what to do', async (t: TestContext) => {
  const dir = await dataDir(t)
...
test('a lock left by a process that is gone is stale: taken over, and said out loud', async (t: TestContext) => {
  const dir = await dataDir(t)
...
test('EPERM on the liveness probe reads as alive, not as gone', async (t: TestContext) => {
  const dir = await dataDir(t)
  const gone = await deadPid()
  await writeFile(join(dir, HOST_LOCK_FILE), JSON.stringify({
    pid: gone, port: 8787, startedAt: '2026-09-10T00:00:00.000Z', hostname: 'somewhere',
...
test('a lock file with garbage inside is stale, and the takeover says it did not parse', async (t: TestContext) => {
  const dir = await dataDir(t)
...
test('release removes the lock, and a host that no longer holds it removes nothing', async (t: TestContext) => {
  const dir = await dataDir(t)
...
test('a recycled pid does not let a shutdown delete another host\'s lock', async (t: TestContext) => {
  const dir = await dataDir(t)
...
test('the lock is created in the data directory even when it does not exist yet', async (t: TestContext) => {
  const dir = await dataDir(t)
