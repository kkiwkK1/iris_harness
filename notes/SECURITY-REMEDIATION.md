...
- **后续（2026-09-17）**：§B 单列的那一行（`install.ts:1182`）已改走 `atomicWriteFile`，§68 的源码网同时补上递归与调用形状（它当初看不见这一行，正是因为不下子目录、又只认具名 import）；性质仍按 §B 判的写：读方不抛、文件派生，是纪律回归不是数据事故。见 app-service §96。
