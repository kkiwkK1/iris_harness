...
### 6.1 暴露面回填(2026-09-02)

上表写于内联脚本这一侧。三处后续测量把它改了,记在这里而不是改上表——**上表是当时
的口径,不是错的,只是不完整**。

**① MVU 本体侧的量纲不同,别和上表相加。** d7 重数了 `.reference/MagVarUpdate`:
显式寻址 **12 处**、`'latest'` 只有 **6 处**,而那 12 处活在 **13/19 张卡**上。所以
「被拒的显式寻址是少数边角」不成立——**它是 MVU 的主寻址方式**。内联 42 处与本体 18
处是两个不同的量纲(前者按调用点、后者按 MVU 源码点),按**覆盖面**读,不要相加。

**② `getCurrentMessageId()` 三处:标结。** 那三处直传当前楼号,曾列为「一开路就是
第一批真实流量」的风险。现已消解:寻址语义已统一为消息下标(见 `DEVIATIONS.md`
与 `tests/variable-addressing.test.ts`,其中一条向量正是这个形状),所以它们开路时
落在正确语义上,不需要迁移。

**③ 渲染后接口楼的用户行:实测为零。** 3e 扫了 **189 个渲染后的接口楼**,用户行
**零命中**。声明含一条卡尺:**73 条 `USER_INPUT` 正则仍然活着,而接口楼里仍然是零**
——也就是说这个零不是「没人处理用户输入」,是「处理用户输入的那些卡不在接口楼里读
用户行」。

**边界:无机制保证。** 上面三条都是**实测缺席**,不是结构性不可能。一张在接口楼读
用户行的卡、或一处对 `characterId` 做算术的调用,都能让对应的零变成一。这一节的数
带日期,推翻它的方式是重跑普查而不是重读这段话。
