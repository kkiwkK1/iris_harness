...
## 97. 按对话的 sidecar，与一个只用来写插件的连接档

### 背景

[SANDBOX-PLUGINS](../../../docs/SANDBOX-PLUGINS.md) §15 的 PR-B，宿主这一半：
`<profile>/sandbox-plugins/<chatId>.json`、三个 `sandboxPlugin.*` RPC、把模型的回答切成一条记录的
两条解析路、以及一个**只为「写插件」存在的连接档**。壳那一半（确认卡、创造模式、列表面板）在
[iris-web DEVIATIONS](../../../notes/apps/iris-web/DEVIATIONS.md) §113。

这里存的东西与这个包里别的任何东西都不同：**它会被执行**。一份读错的设置让一次生成用错温度，
一份读错的插件表让一段对话挂上它没被点头的代码。下面每一条都是从这句话推出来的。

### 决定

**一、一个 chatId 一个文件，不是一个目录。** `cache-trace/<chatId>/` 是目录因为它按 seq 轮转；
这里一次确认要**同时**改版本表和授权记录，两个文件就有一个中间态。路径经 `fileFor`——和聊天文件
同一套白名单与 resolve 后包含性检查，因为 chatId 是从浏览器到达这个进程的。写只有
`atomicWriteFile`，读走 `readJsonStore`，坏文件走 `quarantineCorruptFile`，三条都是既有行为，
一条都没有新写。进程内串行抄 `PluginDataStore` 的 `#chains`：`host.lock` 保证一个 dataDir 一个进程，
**不**保证一个进程内没有并发读-改-写，而这里每一次写都是一次读-改-写。

**二、schema 版本不认识就整份隔离，不迁移、不尽力解析。** 判断写成 `!==` 而不是 `>=`，并且
`>=` 是它的牙齿。理由不是整洁：一份被「尽力解析」出来的插件表，解析出来的是**会被执行的代码**
和**一条授权记录**，而部分读出来的授权是对同意的一次猜测。

**三、`chat.delete` 必须 forget，这是硬要求。** 那一行紧挨着既有的 `settings.forget(chatId)` 与
`chatOrder?.forget(chatId)`，理由比它们都硬一级：chat id 是对着现存文件铸的，删掉之后同一个 id
可以被下一个同名对话拿到——留下的 settings 层改的是一个温度，留下的 sidecar 让**新对话开机就挂上
陌生人的插件，而且是已授权状态**。`cache-trace/<chatId>/` 今天就有这个洞，它在那里的后果只是一份
多余的诊断文件。测试断的是**目录里没有这个文件**而不是 `read` 答空表：一份留下来的文件在 id 被
交给下一个对话之前，`read` 也答空表。

**四、分支复制，带授权，标 `branchedFrom`**（协调人 2026-09-19）。分支的语义是「从这里换一条路
继续玩」，不是「重开一局」；玩家已经对这段代码点过头，分支不是一个新的信任决定。复制而不是按
`parentChatId` 继承，是因为分支的全部意义是两条线各走各的——继承会让子对话的插件跟着父对话的编辑
变，并且父被删除时子会变成 `orphaned`。标记存的是**直接复制来源**：一次分支的分支指向它真正被复制
出来的那个对话，因为那才是面板能说得出名字的那个。

**五、`authoring` 是文件层的一个字段，缺席不回落。** 不进 `GenerationSettings`（那个类型是「一次生成
的路由与采样」，按 chat 合并、按数值区间校验，而「写插件用哪个模型」既不按 chat 分也不是采样参数；
可援引的先例是 `SettingsFile.worldbooks`），不新建第二份供应商列表（它引用一个已有 `StoredProfile`
的 id）。**缺席时「创造」是灰的并说明原因，绝不偷用 `activeId`**：回落会让玩家用一个他没选的模型花
一笔他没预期的钱，而失败的形状（一个写不出插件的对话模型）看起来像功能坏了而不是配置没做。删掉
被引用的那个档时这个字段一并清掉——一个悬空的 `activeId` 让读数变错，一个悬空的 `authoring` 让入口
看起来可用、然后在玩家已经打完那句话之后失败。

**六、请求骑 `#stream` 这一个漏斗，但路由是给定的。** 它是第五个调用方（回合、`script.generateRaw`、
`script.generate`、压缩摘要，现在加上写插件）。`#stream` 多一个可选参数（2026-09-25 起是 `StreamCall.route` 这个具名字段，不再是第五个位置参数）：路由已经定了就别跑那把四级
梯子。这不是绕过漏斗——估算、残留宏报告、指纹、用量回填、side 记账，五个调用方走的是同一段代码——
而是因为**梯子的第 4 级会把悬空的名字换成宿主自己的路由**，那对这条请求恰好就是裁决明令禁止的回落。
适配器装在一个**自己的路由名**（`authoring/<id>`）下，因为 `routeOf` 会让同一个 provider 的两个档共用
一个路由名，而装一个适配器是替换：不这样做的话，选一个写插件的模型会悄悄把这个对话正在玩的连接
重新指向别处。

**七、记账进 `iris_side_usage`，第三个 `SIDE_SOURCES` 成员 `'plugin'`。** 不骑 `'script'`：那是**卡**在
要一次生成，这是宿主替玩家在要，走的是玩家专为这件事挑的档。分不开的话，用量面板会把写插件的钱
报成跑卡片的钱。

**八、两条解析路，一个校验器。** 工具调用优先（代码是一个字段，不用从散文里切），围栏块兜底
（Iris 的连接面是「任意 OpenAI 兼容端点」，不支持工具的端点是常态）。**「供应商支不支持工具」由
回来的东西回答**，不由一张名单回答——这个仓库里没有任何 per-provider 能力位，而一张手维护的端点
名单就是一个会过期的猜测。两条路只在「怎么切出字段」上不同；切出来之后的每一条规则都在
`validateSandboxPluginFields` 里，一条测试把同一份内容顺着两条路各走一遍再逐字段比对。

**九、语法预检用 PR-A 那个共享包装器，在写 sidecar 之前。** `precheckSandboxPluginSyntax` 与帧里的
挂载交给各自编译器的是同一串字节，这是 §6.1 唯一的执行者。预检不过的**不写、不弹确认卡**，句子直接
回给玩家并带一个重试入口——这是七个状态里唯一一个玩家会想「再试一次」的。

### 与上游的对照

**SillyTavern 没有任何对应物。** 上游的一段对话就是一个 `.jsonl`：楼层、swipes、元数据，没有任何
机制让它长出一段自己的代码，也就没有要为它存的东西、没有要为它清的东西、没有一个只为某件事配的
供应商。所以这一节是**有意的改进**，代价逐条写平：

- **`chat.export` 不带它。** 一段在 Iris 里长出了三个功能的对话，导出到 SillyTavern 是一段普通对话，
  再导回来功能不在了。这是[裁决 2](../../../docs/SANDBOX-PLUGINS.md) 的直接后果，也是**兼容是地板**
  在这件事上的具体形态：导出的字节与这个目录不存在时**逐字节相同**，由一条测试钉住。
- **`chat.import` 拿不到它。** 导入会重铸 chatId 并刻意丢掉 `parentChatId`，落地就是一段没有插件的
  新对话。这是正确的：一个来自别的库的 id 指到本地某个对话上，比丢掉更坏。
- **`atomicWriteFile` 不 fsync，所以断电可能丢掉最后一次确认。** 「下次打开还在」是这个功能的卖点，
  而这里能保证的是「旧文件完整」，不是「新插件还在」。卖点与保证之间的这一格写在这里，不是留白。
- **模型写的代码住在 profile 目录里。** 它不是用户自己写的，也不是卡作者写的，而删一个对话会把它
  带走——这是它与 `plugin-data/` 的分别：那里存的是用户数据，卸载插件也留着。
- **写插件那次请求真的花钱，并且花在一个和这个对话无关的档上。** 记账能把它分出来，但它仍然是
  一笔玩家在输入框里打一句话就发生的支出。确认卡上**不显示 token 数**（那个时刻要判断的是「要不要
  这个功能」，一个数字只会挤掉 `purpose`），花费在用量面板里看。

### 与设计稿的出入（三处）

1. **请求体里以前根本没有 `tools`。** §3.2 Q3 的第一条路写着「`translate` 已经把流式 `tool_calls`
   解析成块」——那是**回**的那一半，而且是真的。**去**的那一半不存在：
   `@iris/llm-openai-compat` 的 `serialize.ts` 开头就写着「tool blocks are out of scope because the
   roleplay loop does not call tools」，`serializeRequest` 里没有 `tools` 也没有 `tool_choice`。
   所以这条路整条是不可达的。最小的忠实落法是在那一个文件里加六行：**只在调用方声明了工具时**才发，
   于是别的每一条请求的字节与改前逐字节相同——这件事本身是承重的，端点的前缀缓存是按发出去的字节
   算的，给每一个回合加一个空数组会把语料里每一条前缀都挪位。
2. **`sandboxPlugin.list` 的回包多一个 `mounts` 数组**，见 web §113 出入 1：§10.2 禁止视图带 `code`，
   §4.1 第 12 步要求壳把 code 发进帧，两句话只有这一种同时成立的读法。
3. **「这张卡的界面结构摘要」窄成了「这张卡的脚本叫什么」。** §4.1 第 5 步要的是界面结构，而宿主
   没有 DOM——界面是消息里的 markup，在浏览器里。宿主手上真实存在的、和插件会撞车的东西是**同一个
   realm 里还跑着谁**，所以请求里给的是卡的脚本名单，经 `script.list` 用的那同一个 helper 取，
   两处不会描述出不同的集合。

### 测试与牙齿

九次变异，每次**改代码**（不是改夹具），各自至少红一条，恢复后全绿。

| 断言（在哪） | 让它变红的改动 | 结果 |
| --- | --- | --- |
| 两条解析路对同一份内容产出同一条记录（`sandbox-plugins.test.ts`） | 把围栏路的代码块标记从 `js` 改成 `javascript` | 红 1 → 绿 |
| 七个上限各一条，`codeBytes` 写成字面量 | 把 `64 * 1024` 改成 `64 * 1000` | 红 1 → 绿 |
| schema 版本不认识即整份隔离 | 把 `!==` 改成「`>=` 就收下」 | 红 1 → 绿 |
| **`chat.delete` 之后 sidecar 文件不在了** | 注释掉那一行 `sandboxPlugins?.forget(chatId)` | 红 1 → 绿 |
| 分支带着授权与 `branchedFrom` | 跳过分支时的复制 | 红 1 → 绿 |
| 同上 | 把 `branchedFrom` 标成子对话自己 | 红 1 → 绿 |
| **没有 authoring 档时拒绝，且一个请求都没发出去** | 让 `authoring()` 在缺席时回落到 `activeId` | 红 1 → 绿 |
| 未授权的版本不进 `mounts` | 把 `isPluginAuthorized` 那一格关掉 | 红 1 → 绿 |
| **`chat.export` 的字节与没有 sidecar 时逐字节相同** | 让 export 顺手把插件表拼在后面 | 红 1 → 绿 |

两条值得单独说，因为它们第一次都是**绿的**，而绿的原因不是代码对：

- **七个上限**那条最初把每个数字都从 `SANDBOX_PLUGIN_LIMITS` 读出来，于是把 `64 * 1024` 改成
  `64 * 1000` 之后比较的两边一起动，测试照绿。一个只对着自己断言的上限，是一个没有被钉住的上限。
  现在 `codeBytes` 写成 `65_536` 这个字面量，其余仍读常量——不对称是刻意的，这一个是设计裁下来的数，
  别的几个是关系。
- **不回落**那条最初的夹具没有 `markActive`，于是「回落到 `activeId`」这个变异里 `activeId` 本身就是
  空的，变异是惰性的。现在两个夹具都把档设为在用——包括那个**故意不设 authoring** 的，因为这条
  断言要分辨的正是「没有 authoring」和「回落到在用的那个」，而一个什么都没在用的宿主分辨不出来。

第七条的判别力在于它断的是**发出去的请求**而不是抛出的错：一个回落之后因为别的原因失败的实现也会
抛错，而裁决禁止的是那笔支出，不是那个异常。

### What would reopen this

(a) 工具循环立项：它的 sidecar 应当和这一份共用同一个位置决定（§13.4 已经把这条留给协调人），
不共用的话一次备份要知道两个地方。
(b) 出现一个真的不接受 `tools` 字段并直接 400 的端点：今天「支不支持工具」由回来的东西回答，
那时要在**发之前**知道，而知道它的唯一诚实办法是先失败一次再记住——那是一条新的状态，要设计。
(c) 插件开始需要跨对话复用：`<chatId>.json` 这个形状就是对「一个插件属于一段对话」的编码，
届时整个 §10 重写，`forget` 那条硬要求的理由也要重读。
(d) 第一批真实使用之后：§10.4 的五个上限全部是**判断不是测量**（这个功能当时还不存在，没有语料
可量）。量一次真实插件的体量与一段对话真实长出来的个数，然后复核这五个数。
(e) `iris_side_usage` 长出第二个非回合的新成员：`SIDE_SOURCES` 已经三个了，第四个之前应当先问
用量面板要不要把它们收成一组，而不是一行一行加下去。

## 98. 一次单独的源码读，与用量里的第三条侧支

### 背景

[SANDBOX-PLUGINS](../../../docs/SANDBOX-PLUGINS.md) §15 的 **PR-D**，宿主这一半：
`sandboxPlugin.source` 这一个 RPC，以及把 `'plugin'` 这笔钱从「算进总量」变成「能被单列出来」。
壳那一半（只读视图、空态、用量页那一行）在
[iris-web DEVIATIONS](../../apps/iris-web/DEVIATIONS.md) §116。

两件都很小，但各自修的是一条**只在读数上才看得见**的缺口：源码本来只能靠把它塞进 `list` 的
回包才拿得到，而那是 §10.2 明令不做的；插件那笔钱本来计进了总量却没有任何一处能把它拆出来。

### 决定

**一、`sandboxPlugin.source` 是一个读，不是 `list` 的一个可选字段。**
`{ chatId, pluginId, version? } → { code, hash, version }`，静态 schema，与另外三个同处。
`version` 缺席即当前版。**答的是存着的字节，不重新包一层**：`hash` 正是对这些字节算的，
读者看的是他点过头的那一份。`version` 是**答出来**而不是回显的——请求可以不带它，
而「当前那一版」不说出名字就没有用。

**二、id 作用域就在 `read(chatId)` 里，处理者不写第二次检查。**
一个插件属于持有它那张表的对话，所以拿另一段对话去问这段对话的插件，是找不到、具名拒绝。
处理者里**没有**「这段对话拥有它吗」的第二道检查，因为那会是同一个答案的第二个来源，
而且是容易忘记维护的那一个。代价写平：这条断言因此没有属于它自己的牙齿——
它和「没有这个插件」共用同一句 `notFound`。能让它单独红的改动在 store 那边
（让 `#fileFor` 忽略 chatId），牙齿就记在那里。

**三、要不到的版本具名拒绝，绝不回落到当前版。**
理由与裁决三（确认卡按哈希授权）同源：一个问 v2 却被悄悄递了 v3 的读者，手里拿的是错的字节，
而屏幕上没有任何东西会说出这件事。拒绝的句子把**还留着哪几版**一起说出来，
因为最常见的原因是最旧那版被挤掉了（留 8 版，§10.4），不是这个号从来不存在。

**四、`'plugin'` 折进自己那一格，做法是把守卫多写一个词。**
`addUsage` 里那个 `if (source !== 'script' && source !== 'compaction') return` 旁边，
PR-B 留的注释写着「第三个侧支靠放宽这个守卫来接入，而不是靠谁记得再抄一条分支」。
`'plugin'` 是第一个来兑现这句话的，而它确实只花了一个词。
协议侧配套的是 `UsageTotals.plugin?: UsageBuckets` —— **PR-B 没有加这个字段**，
所以在这之前这笔钱计进了总量、没有任何一处能把它单列出来。

**为什么不并进 `script`**：那是**卡**在要一次生成，这是宿主替玩家在要，
走的是玩家专为写代码挑的那个档（§97 决定七已经为记账写过同一句）。
一个嫌这个数大的读者要改的是那一行的模型或者自己的习惯，不是一张卡。

**五、缺席仍然是缺席。** 从没长出过插件的档，`totals.plugin` 是 `undefined` 而不是一排 0——
与另外两条侧支同一条规则，也是界面上「整句不画」的依据。

### 与上游的对照

SillyTavern 没有对应物：没有「这段对话自己长出来的代码」，就没有读它、
也没有为它单独记一笔账的问题。代价：

- **`sandboxPlugin.source` 是这台宿主上唯一一条把模型写的源码交给浏览器的读**
  （`mounts` 交的是要跑的那些，是另一回事）。它一次只答一个版本、必须具名、必须对上对话，
  这三条就是它全部的收口。
- **`UsageTotals` 的可选份额字段已经三个了。** 每一个都与 `TurnUsage.source` 的拼写逐字相同，
  合计器因此按 source 取键而不是按分支写死；但**界面那边是三句手写的话**，不是枚举出来的。
  第四个成员之前该先回答 §97 (e) 那个问题。
...
| 别的对话读不到这个插件（同上） | 让 store 的 `#fileFor` 忽略 chatId | 红 → 绿 |
| `'plugin'` 折进自己那一格（`usage-summary.test.ts`） | 守卫改回只认 script/compaction | 红 → 绿 |

`sandbox-plugins.test.ts` 里那条 `source` 的测试**自己造第二版**，
不是靠第二次 `define`：夹具只答一个写死的回复，两次 define 会得到两份逐字节相同的版本，
于是「v1 与 v2 答不同的字节」这条断言对着一个**完全忽略 `version`** 的实现也会通过。

### What would reopen this

(a) 源码开始需要成批读（并排比版本、导出一段对话的全部插件）：
那时「一次一个版本」要重看，而它今天买到的正是「壳里不会攒下一份不可信文本」。
...
## 101. `chat.resync`：只读地回答「这条对话落定了没有」，给丢了事件的页面

**Kind:** Iris-only addition（主人 2026-09-24 00:05 卡住的回复）。web 账本 §124 是页面那半，原因、
读数与测试表都在那里；rpc-host 账本 §3 是挂断留痕。编号待协调者重排。

**为什么要一个新方法，而不用 `chat.open`。** 页面在事件断档（重连、或套接字不再送帧）之后要问的
只有两件事：已落定的 view 是什么、这个对话此刻有没有在途的 turn。`chat.open` 答得了前一件，答不了
后一件（`ChatView` 里没有「正在生成」），而且它有三件副作用——向扩展平面宣告对话
（`#stCompatAnnounceChatOpen`，平面的预载重跑）、重新广播旧版清理的询问、对重开的对话记「注入
仍在」的 note。对一个只是丢了几帧的页面，这三件都不对；页面的静默看门狗还可能在慢提供方思考时
每 20 s 问一次。所以新方法只读。

**形状**（`rpc.ts`）：请求 `{ chatId, reason: 'reconnect' | 'silence', streamTurn?, silentMs? }`，答复
`{ view, generating?: { turn } }`。view 与在途 turn 在同一个同步步骤里读（唯一的 `await` 在它们之前），
所以不会有落定夹在两者之间：答「不在生成」的那份 view 一定已经含着回复。

**唯一的写：一条 note。** 页面报来的 `streamTurn` 不是宿主此刻在生成的 turn（多半是宿主早已结束）时，
记一条 `host` 类 `note`，句子说页面因为什么来问（重连 / 多少秒没有流帧）、它当时显示哪个 turn 在生成、
宿主的实际状态。这是「页面本来会一直卡着」的可数痕迹；干净的重连（页面没有显示任何生成中）不记。
`tests/chat-resync.test.ts` 三条：在途 → 答 `generating` 且不记；落定 → view 含整条回复、记一条；
没卡住的页面来问 → 不记。改坏宿主让它永远答「不在生成」→ 第一条红；让 note 无条件记 → 第一、三条红。

**何时重开：**(a) `ChatView` 自己带上在途 turn：那时 `chat.resync` 可以退化为「不带副作用的
...
## 103. 行的持久 id（`iris_id`）、分支点 `iris.branchAt`，与只读的 `chat.tree`

**Kind:** Upgrade（主人 2026-09-25 裁定 6：持久行 id；同日的分支树需求）。web 账本 §125 是树图、楼层徽标与「转成分支」那半。
编号待协调者重排。

**上游怎么做。** SillyTavern 的分支是「一个分支 = 一个聊天文件」：`bookmarks.js` 把 `chat.slice(0, mesId + 1)` 存成新文件，
子文件头里 `chat_metadata.main_chat` 写父聊天的**名字**，父聊天那一楼的 `extra.branches` 追加子聊天的名字。它不记分支是从哪一楼切的
（只能从父那一楼的标记倒推），没有行 id，没有树视图。

**Iris 的做法，三件事。**

1. **持久行 id。** Iris 自己写出的行带一个顶层键 `iris_id`（`@iris/persistence` 的 `LINE_ID_KEY`），值是铸出来的随机 UUID，
   **不是**从楼层号或文本推出来的——楼层号在上面一行被删时会移，文本在每次编辑、每次换读法时会变，两者都答不了「这是不是同一行」。
   放在顶层、不放进 `extra`：`extra` 是别的扩展往里写的地方；上游对不认识的顶层键原样带着存。
   **从文件导入、没动过的行一律不加**（判据：这行有导入时记下的键序，每一条导入行都有），所以导入后原样导出仍然逐字节相同。
   唯一给导入行补 id 的地方是分支点：那一行本来就要被改（加 `extra.branches`），一并补上。
   铸出的 id 在同一个会话里按消息事件记住（`WeakMap<Session, …>`），第一次保存后它就是 `iris/st-meta` 里的普通携带字段，之后每次加载都读回同一个。
2. **分支点记下来。** `chat.branch` 在子文件头的 `iris` 块里写 `branchAt: { floor, lineId }`。`readMeta` 读它，
   否则 `touch()`（每一轮都会从 `readMeta` 重写整个 `iris` 块）会在分支的第一轮把它抹掉——测试钉住了这一条。
   上游的 `main_chat` 与 `extra.branches` 照写不误，所以分支导回 SillyTavern 仍然知道父是谁、从哪一楼走的。
3. **`chat.tree(chatId)`，只读。** 从 `chat.list` 的父链接（已含对 `main_chat` 的解析）先算出家族——根祖先加它的所有后代——
   **只读这些文件**，交给纯函数 `chat-tree.ts` 建图：每个聊天的楼层数、每楼的读法数、分叉点、当前聊天及其最新楼层。
   分叉点按强度取第一个有的：`recorded`（`branchAt`，先按 `lineId` 在父里找那一行，找不到才用楼层号，夹到父的长度内）→
   `marker`（父某一楼的 `extra.branches` 点名了这个子聊天，取最后一个）→ `prefix`（两边逐楼比「角色 + 文本」的最长公共前缀；
   若分歧那一楼两边的**读法集合**相同而选中的不同，判为「这一读法转成了分支」，分叉点就在那一楼）。
   父不在了（被删、或上游名字从未解析）的聊天自成一棵树的根，节点上写 `detachedFrom`；手改文件造成的父环在第一个回头的边上剪断。

**测量。** `tests/line-id.test.ts`：导入未动 → 逐字节相同且不含 `iris_id`；追加一轮 → 新两行有互不相同的顶层 id、导入的三行没有；
同一日志存两次 id 相同，重载再存逐字节相同。`tests/chat-tree.test.ts` 14 条：旧分支（前缀）、没往下走的旧分支、旧的读法分支、
标记胜过前缀、`lineId` 追踪父里移动过的行（只用楼层号会答 4，答案是 2）、`lineId` 丢失时夹到父长度、嵌套、父缺失、
中间一个被删、不可读文件、父环、家族只由列表行决定、未知聊天。`tests/branch.test.ts` 新三条：`branchAt` 与父那一行的 `iris_id`
相同且活过分支的第一轮；真宿主上根 → 分支 → 分支的分支 + 读法分支，从任一成员问都得同一棵树；手写的上游旧分支按前缀推断，且两个文件字节不变。
牙齿：「永不铸」→ line-id 红 1；「导入行也铸」→ 红 4；`readMeta` 丢 `branchAt` → branch 红 2；去掉 `lineId` 查找 / 读法规则 /
标记规则 / 环剪断 → chat-tree 各红 1。

**代价。** Iris 写的每一行多约 50 字节（`"iris_id":"<36 位>"`）。`chat.tree` 每次调用读并解析家族里每个文件（不缓存）；
页面只在楼层数或列表变化时重取。
...
## 106. 删除一段对话时，它的子分支接到上一层，而不是悬空

**Kind:** Upgrade（主人 2026-09-26 的分支管理需求：删分支、重命名、每分支一色、按活跃度定线宽）。web 账本 §129 是树图上的删除、
重命名、颜色与线宽那半。编号待协调者重排。

**上游怎么做。** SillyTavern 删聊天只删那一个文件（`/api/chats/delete`）。从它切出去的分支文件头里 `chat_metadata.main_chat`
仍写着被删的名字，于是它们成了指向不存在父亲的孤儿；下一段恰好同名的聊天还会被这个名字「认领」。Iris 在本条之前也一样：
`chat.delete` 删文件、清按 id 分区的存储（裁定 5），子分支的 `iris.parentChatId` 指向空处，树图把它们各画成一棵「来处已不在」的根。

**Iris 的做法。** `chat.delete { chatId, subBranches? }`，`subBranches` 缺省为 `'reattach'`。删之前先读这个家族的文件（与 `chat.tree`
同一套：`lineageOf` 决定家族，只读家族的文件），交给纯函数 `planBranchDelete`（`chat-tree.ts`）定好计划，再动盘：

1. **中间的分支：** 只删它。它的每个子分支改挂到它的父亲下，分叉点重算：子分支在被删分支的第 N 层切出，而被删分支在父亲的第 M 层
   分出去，所以子分支前 min(N, M) 层就是父亲的原样；新的 `branchAt` 取 `{ floor: min(N, M), lineId: 父亲那一层的 iris_id }`。
   分叉判定仍走 `forkOf` 的既有规则，所以被删分支若是「读法转成的分支」，接过来的子分支那一列从分叉层本身开始（与父亲读法不同）。
2. **删根：** 选择「提升」而不是「拒绝」。根的第一个子分支（按树图的列序：分叉层最早，其次最久未动）成为新根——它就是树图里紧挨着
   主干的那一列，读者一眼看到的「下一条」；其余子分支挂到它下面，分叉点取 min(自己的分叉层, 新根的分叉层)。拒绝会让一个被分支过的
   对话永远删不掉，而边栏的「删除对话」正是这种情况。
3. **`'delete'`：** 删它和它所有后代（父先于子）；什么都不改挂。

改挂只改文件头：`iris.parentChatId`、`iris.branchAt`，以及上游的 `chat_metadata.main_chat`（写新父亲的标题，与 `chat.branch` 同一写法；
提升成根的删掉这个键，免得日后被同名的陌生对话认领）。**`updatedAt` 不动**——别处的删除不是这段对话的活动，动了会改边栏次序、也会把
树图上这条线描粗。没打开的对话只重写文件头那一行，每一楼的行逐字节不变；打开着的在活对象上改再存，免得下一次保存把旧血统写回去。
顺序是先改挂、后删除：改挂失败则家族原样；删除失败只剩一段没有孩子的对话。备份照旧保留（裁定 5）。

答复从 `{}` 变成 `{ deleted, reattached, promoted?, successor? }`：`successor` 是读者若正在读被删的那段该去的地方（父亲，否则被提升的新根）。
假客户端用同一套规则（`fakeDeletePlan`）。

**测量。** `tests/branch-delete-plan.test.ts` 6 条（纯函数，经 `buildChatTree` 验图）：中间分支的两个孩子——切在它分叉层之上的保留原分叉（2），
切在它自己楼层上的改为它的分叉（4），且因为它是读法分支，接过来的孩子 `shared` 为 4；孙子不动；切在同一层的；无记录分叉的旧分支被补记；
删根提升第一个孩子（分叉 2 的「early」，不是更早动过的「late」）；叶子与单独的根；`'delete'` 连子树。
`tests/branch-delete.test.ts` 5 条（真服务、真文件）：中间分支删后 `chat.tree` 里 C 在 1、B 在 2、D 仍是 B 的孩子，没有 `detachedFrom`；
B 的楼层行逐字节不变、`updatedAt` 不变、`main_chat` 改为根的标题；删根提升；`'delete'` 之后盘上只剩根；未知 id 仍是 `not-found`；
重命名在列表与树里都生效、孩子仍挂着（链接是 id 不是标题）。
牙齿：分叉取「孩子自己的」而不是 min → 红 4；不写 `main_chat` → 红 1。
真浏览器验收 `qa/branch-manage-acceptance.mjs`（8804，拷贝的数据目录，不生成）全部通过，见 web §129。
...
## 108. 分支段总结：按需生成、走会话自己的路由、记为旁路用量，存 Iris 侧车，不进 ST 文件

**Kind:** Upgrade（主人 2026-09-26 的需求：每个分支段都有一段模型写的总结，悬停时显示）。web 账本 §133 是树图上的悬停卡片与
「总结所有分支段」那半。编号待协调者重排。

**上游怎么做。** SillyTavern 没有分支树，也没有「一段楼层的总结」。最近的是 Summarize 扩展：它给**整段对话**写一份滚动摘要，
存进 `chat_metadata` 并注入提示词。本条既不注入提示词，也不写进聊天文件，所以不与它冲突，也不改变任何一次回合发出的请求。

**Iris 的做法。**

- **什么是一段。** `@iris/protocol` 的 `segmentsOf(tree)`：只读 `chat.tree` 的图。每段对话拥有自己那一列的楼层
  （`fork.shared` 到最新一层；根从 0 起）；每条分支在「拥有它离开的那一层」的祖先列上切一刀——普通分支切在分叉层之后，
  读法转成的分支（`shared = floor`）切在分叉层之前，因为两列在那一层本身分开；什么都没写的分支也切，但不拥有段。宿主与网页
  用的是同一个函数，所以悬停卡片说的楼层就是宿主总结的楼层。黑兽（0–32，Branch #1 从 30 分出）是三段：根的 0–30、根的 31–32、
  Branch #1 的 31–32。
- **两个 RPC。** `chat.segmentSummaries { chatId }`（只读）答这个家族每个**已有总结**的当前段，带 `stale`；
  `chat.summarizeSegment { chatId, fromFloor, toFloor, lane? }`（写）要求 `fromFloor..toFloor` 恰好是一段，否则
  `invalid-request`，一个请求也不发。`lane` 指明段属于哪一列（悬停的是兄弟分支的段时）；缺省时取 `chatId` 往根走、拥有
  `fromFloor` 的那一列。二者都有 typed PROBES 条目与假客户端分支（假的答一句固定的总结）。
- **模型调用。** 走唯一的漏斗 `#stream`：**没有 `entry`**（卡片模板不在宿主写的提示词里求值；校准器与回合的 `actualTokens`
  不动，只有回合可以动）；**有 `side`**，`source: 'segmentSummary'`、`caller: 'chat.summarizeSegment'`，记在提问那段对话的
  文件头上，用量页多一个份额；路由是这段对话自己的（与压缩相同的解析）。**没有 `trace`**——这是与压缩的有意不同：压缩的请求体
  解释「为什么以后每一回合的前缀变了」，段总结不改变以后的任何请求，而「总结所有分支段」一次发 N 个，会把每段对话保留的 8 份回合
  trace 全部挤掉。发出的只有这一段的楼层（隐藏楼层跳过、每层带说话人名字），不是根段时前面加一行「此前」（上一段已有总结的第一句，
  否则一句「前面的楼层未给出」），最后是 `SEGMENT_SUMMARY_INSTRUCTION`：2–4 句、用对话的语言、不续写不编造——语言与不编造两条
  规则与压缩指令共用常量，压缩指令逐字节不变。不发对话的系统提示词（主人：只发这一段），所以也没有可与回合共享的前缀。
  上限 2048 token，截断即拒绝（与压缩同理：半截的总结比没有更糟）。
- **可取消。** 登记在按条目的旁路调用登记表：提问那段对话的 Stop（`chat.abort`）、删除或恢复会中止它；段属于别的对话时也在那段
  对话上登记，删掉它也会中止（`AbortSignal.any`）。被中止的请求按名拒绝，不存任何东西；中止前供应商已报的用量照记（`#stream` 的
  `finally`）。
- **存储。** `segment-summaries/<chatId>.json`，一段对话一个文件，文件名是**拥有**该段楼层的对话。每条记录：首层与末层的行键、
  所有楼层「角色 + 显示文本」的哈希、生成时的楼层号（只用于显示）、总结、时间、模型。行键是持久行 id `iris_id`（裁定 6）；
  导入后从未动过的行没有 id，退回 `@<chatId>#<floor>`（按位置——没有身份的行只能如此）。**匹配规则**：首键、末键、哈希都相同为
  新鲜；否则取首键相同中最新的一条，标为过期（编辑了一层、换了读法、段变长——活对话的末段每回合都在长——或被新分支切成两段）；
  首层不是任何记录首层的段没有总结。读的时候合并**家族所有成员**的文件再匹配，所以共同前缀只写一次（在根上），从任何一条分支都
  读得到。
- **删除。** `chat.delete` 调 `segmentSummaries?.forget(chatId)`，删这段对话拥有的文件；备份保留（裁定 5）。删角色保留
  （对话比卡片长寿，总结跟着对话走）。`tests/entity-lifecycle.test.ts` 的表里加了这一行。

**测量。**
`packages/iris-protocol/tests/segments.test.ts` 8 条：只有根；一次分叉（黑兽的形状）；嵌套——孙子从父亲自己的楼层切出，切父亲；
孙子从父亲**抄来**的楼层切出，切根（最近的错误实现会切父亲、让根的 0–9 保持整段）；读法分支在分叉层本身分开；什么都没写的分支
切父亲但不拥有段；同一层两条分支只切一次。
`packages/iris-app-service/tests/segment-summaries.test.ts` 9 条（真服务、真文件、假 provider 每次都报用量，总结与回合的数字
每个桶都不同）：非一段的范围被拒且未发请求；记为 `segmentSummary` 旁路用量、`usage.summary` 有该份额、校准样本数不变
（前提：回合确实喂了 ≥ 3 次）；只发该段楼层，后一段带「此前」一行且含上一段总结的第一句；共同前缀从分支提问也记在根上、
只一条，两段对话读到相同列表；编辑段外一层不过期、段内一层过期、重新总结后新鲜且不留旧记录；段变长读作过期而不是消失；Stop 中止
进行中的请求且什么都不存；删除分支删掉它的文件、保留根的；匹配规则本身。
牙齿：给 `#stream` 传 `entry` → 校准那条红（"the segment summary moved the turn calibration"）；`source` 改 `'script'` → 红；
去掉 `forget` → 删除那条红；`owner` 改为 `chatId` → 共同前缀那条红；去掉 `signal` → Stop 那条超时红。
`packages/iris-client-fake/tests/branch-tree.test.ts` 新增 1 条：假客户端答固定总结、从家族读得到、非段边界被拒。
真浏览器验收 `qa/segment-summary-acceptance.mjs`（8806，拷贝的数据目录，本地 mock provider，**不花钱**）全部通过，见 web §133。

**何时重开：**(a) 主人要总结进提示词（像上游 Summarize 那样注入）：那是另一个功能，会改变回合请求，需要自己的条目。
(b) 删根后被提升的孩子拥有了原来根的前缀楼层，但根的文件已随删除遗忘：前缀段要重新总结。若主人要保留，就在 `planBranchDelete`
的提升路径里把根文件里首键仍在孩子身上的记录搬过去。(c) 分支在自己抄来的前缀里编辑了一层：树图仍把那段画在根上，根的总结照旧
新鲜，因为分叉判定（`forkOf`）只比较分叉层——若要按分支自己的前缀判定，需要先改 `chat.tree` 的分叉规则。
