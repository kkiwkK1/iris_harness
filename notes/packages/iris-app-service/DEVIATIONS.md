...
pulls in — does for the same set for the same reason. The deterministic test for
it holds a read handle open on the target and releases it after 80 ms; on a
POSIX host that rename succeeds immediately and the test passes without
exercising the retry, which is said out loud in the test rather than left to be
discovered.
