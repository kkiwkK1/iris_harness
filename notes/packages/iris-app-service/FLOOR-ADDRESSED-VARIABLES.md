...
> 状态：记录。测量于 2026-09-07，对象 语料 27 张卡的脚本正文与最长一局，经产品自己的读取器。记录不随代码更新；Iris 侧的现状以 `docs/` 为准（索引见 `notes/README.md`）。
...
2. ~~**18.32 MiB 快照的实际传输代价**~~ —— **已量,见「七」**:构建 224.3 ms + 克隆
   85.0 ms = 每次快照 309.3 ms 主线程阻塞;而变量表占字节 45%、占克隆时间约 91%。
3. 卡正文普查的两条失明(§三)意味着**读/写点数永远只有下界**,没有上界。
4. 本文数字单人单次,未复核。

---

## 七、快照的传输代价(补测,2026-09-03)

「二」里那笔账缺的分母,量出来了。最长那局(677 行 / 18.32 MiB 的 `chat` 数组),
中位数(5 次):

```
toFile() 构建           224.3 ms    ← 宿主每次快照都付
structuredClone          85.0 ms    ← postMessage 实际付的
JSON stringify+parse     71.3 ms    (作对照)
合计                    309.3 ms    主线程阻塞
```

**而把 variables 从行上摘掉之后:**

```
clone(不带 variables)     7.6 ms   ( 10.02 MiB )
clone(带 variables)      85.0 ms   ( 18.32 MiB )
```

> **变量表占字节的 45%,却占克隆时间的约 91%。**
> 克隆的代价跟**对象个数**走,不跟字节走:`mes` 是几个大字符串,便宜;变量表是成千上万
> 个小对象,昂贵。

**「均值骗人」在这一节又出现了一次,而且换了个维度**:上一次是字节在楼层间分布不均,
这一次是**时间和字节根本不成比例**。按 MiB 估传输代价会把这件事估反。

### 这对刚下的裁定意味着什么

裁定(1)让 frame 从 `context.chat[N].variables[swipe_id]` 同步回答——**今天是对的**
(数据已在手,不需要新传输),但它依赖的正是这 8.29 MiB。所以:

- **短期**:实现零成本,因为这笔钱已经在付了。
- **但 WINDOWING 那一步一旦把 variables 从 `chat` 里摘出去,楼层读就同时失去依据。**
  两件事绑在一根绳上:**摘表和改楼层读法必须同批**,否则中间态是「卡拿不到楼层变量」,
  和内嵌书那次「先拆后补」是同一个形状。

这条现在写下来,是因为它在两份文档之间——摘表的动机在这份,代价在 WINDOWING 那份,
而**中间那根绳子不写下来就没人看得见**。
