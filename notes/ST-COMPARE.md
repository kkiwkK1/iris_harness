...
> **坐标:本表定格于 Iris `3cf858d`(2026-09-05 01:09,`dev/iris-exploration`)。** 它是一份
> 对那个时刻的测量,不随后续提交更新:至少八行差距(persona / 宏词表 / 聊天导入导出 /
> 聊天搜索 / 全局正则层 / 角色管理 / 生成类型 / 世界书条目编辑器)已被 09-05 至 09-06 的
> 提交关掉,逐个 `git merge-base --is-ancestor` 核过(见 `MD-INVENTORY.md` §六)。要知道
> 「现在」的差距,按新 tip 重测,不要读这份表的状态列。
