...
> 状态：已被取代（2026-09-16 记）。本文描述的工作随 PR #88（`2eccf30`）进入 `main`，当前缺口以 `docs/INFRASTRUCTURE-INTERFACES.md` §8 为准；「必须保持的行为合同」一节仍然有效。因源码注释引用本文件路径，留在原地。下面这段 2026-09-15 的状态块与正文原样保留。

> **状态（2026-09-15）：本文描述的工作已经落地。** 系统插件平台与 ST 扩展试点在
> PR #88 合入 `main`（`2eccf30`），两份受影响的接口文档在 PR #89 重记基线
> （`1383a12`）。因此：
>
> - 下面的「与 Claude 的边界」分工表、「接手顺序与重点检查」清单、「已有验证证据的
>   范围」与「交回验收时提供」都是**历史记录**，用来回溯当时为什么这样切分，不是
>   待办事项；其中「尚未提交、尚未整体验收」「没有整树最终 build/full test/no-corpus
>   通过结论」等句子说的是 2026-09-12 那天的现场，今天已不成立。
> - 现场信息同样作废：`D:/workspace/小项目/iris-system-plugins`、分支
>   `dev/system-plugins`、保存点与 stash 都已随合入退役，**不要**照着它们操作。
> - 「必须保持的行为合同」一节仍然有效——它是行为约束，不是进度，验收结果见
>   [插件平台联合验收](PLUGIN-PLATFORM-ACCEPTANCE-2026-09-13.md) 与
>   [ST 试点验收报告](st-compat/PILOT-REPORT.md)。
> - **当前还缺什么，以 [docs/INFRASTRUCTURE-INTERFACES.md](../docs/INFRASTRUCTURE-INTERFACES.md)
>   §8 为准**；本文不随版本更新，它的沉默不代表新闻。
>
> 正文以下原样保留，未作改写。
