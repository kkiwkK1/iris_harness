...
> 状态：记录。测量于 2026-09-02（入库 2026-09-07），对象 上游 SillyTavern 1.18 消息正文的内联 HTML 地板，与 Iris 的收紧方案。记录不随代码更新；Iris 侧的现状以 `docs/` 为准（索引见 `notes/README.md`）。
...
## 八、落到「帧里」的那一版：实施记录（2026-09-07，样式代理）

这一节只记**已经落地的东西**和它与 §4.4 的两处偏离，不改上面任何结论。
走的仍是 §三 被否掉的那条路（帧路，不是内联路），所以 §4.4 那张表的前提
「一条消息一棵树」在这里**不成立**，选型结论要跟着变。

**做了什么。**一条消息里所有不在围栏内的 `<style>` 块不再各自成为一个 region /
一个帧：`splitHtmlRegions` 把它们作为 `MessageStyle`(span + CSS) 返回，
`claimMessageSurfaces` 合并后经 `card-css.ts` 收敛一次，
`runMessageInterfaces` 把它挂到**每一个 bare-html region** 的 markup 上，
`buildSrcdoc` 再把它抬进那一帧的 `<head>`；同一批 span 交给
`splitAroundInterfaces` 作为「丢弃」，所以这些字符既不进帧也不进 prose
（`MarkdownText` 关掉了 raw HTML，漏进去就是一屏 CSS 源码）。

**为什么必须复制到每一帧。**帧之间互不可见，一帧里的规则碰不到隔壁帧里的
`<details>`。上游不需要复制，因为一楼层就是一棵树；在一 region 一帧的模型下，
**复制就是「消息级作用域」的唯一等价物**，不是可以以后优化掉的权宜。

**偏离 §4.4 之一：作用域根是 `body`，不是 `#msg-<候选序号>`。**
帧里只有这一条消息的这一个 region，候选序号要挡的「跨消息」在帧边界处就已经挡住了；
`body` 是上游 `.mes_text ` 前缀在这个文档里的等价写法。
量过：语料里 13 份消息级样式**没有任何一份**写 `html` / `body` / `:root` 规则，
也没有任何一份去抓 `.mes_text` / `#chat`，所以 `@scope` 根本身不可被自己匹配这件事
碰不到它们。

**偏离 §4.4 之二：`@keyframes` 不再重命名（`KeyframePolicy: 'keep'`）。**
§4.4 第 1 条是对的——**在共享 DOM 里**。在帧里没有第二张卡可撞，而重命名有东西可坏：
量到 **13 份里有 5 份**的 `@keyframes` 是被 markup 的 `style=` 属性引用的
（爱衣 `shimmer`；可攻略女主与暗渊的 `moon-halo` / `moon-pulse` / `stars-drift` /
`moonlight-sweep`），而改写只覆盖**同一张表内**的 `animation` 声明——
重命名等于让那些 `style=` 引用一个再也没人定义的动画，**不报错，只是不动了**。
表内自引用（`contentSlideIn`）仍然被一起改写，仍然正确。
`@keyframes` 该被提到 `@scope` 外面这半条**照做**，与名字无关。
§4.3 的拒绝清单一条没松：`@import` / `@font-face` / `@namespace` / `@charset` /
一切 `url()` 外部取用，在帧里的表和在消息里的表一样拒——语料这 13 份今天一条都没被拒。

**影响面（口径：本仓 `claimMessageSurfaces` 读，25 张卡，220 条候选文本 = 一条
`replaceString` / 一条 `first_mes` / 一条备选开场）**：13 条文本、9 张卡是
「HTML 片段 + 一个空行 + `<style>`」，全部是变量更新美化，全部出自 `replaceString`；
改后语料里只剩 **0** 个纯样式帧，这 13 条各出 1 帧（原来 2 帧）；
13 份表收敛后共 11.9 KiB（均 ~940 B）。

**剩余缺口，写在这里是因为它不是 bug 而是模型的边界**：跨 region 的选择器
（`.a ~ .b`、`.a .b` 两端分属两个 region、跨 region 的 `:has()`）在任何一帧里都匹配不到——
表在每一帧都有，但每一帧只有自己那段树；消息级样式也再也碰不到消息的叙事正文。
上游都能匹配，因为一楼层一棵树。语料这 13 份都不这么写（每份都只选自己那个片段内部）。
**要修就只能是 §三 那条内联路**，在那条路上这个缺口自己消失。
详见 `DEVIATIONS.md` 第 51 条。
