...
**后续：** 「哪几种引号算对白」在 §122 里被改了——默认从上游的六种收到三种说话引号，
本节其余部分（正则形状、标签保护、少一个 `s`、引号留在 `<q>` 里面、落点在渲染之后）一字未动。

---

## 122. 默认只给对话上色：`「…」` 与 `『…』` 在中文里多半是术语，不是对白

**Kind:** deliberate divergence, with a setting back to the floor；规则形状照抄，**取哪几对引号**是 Iris 自己的

**上游把六对引号一视同仁。** `[ST] public/script.js:1846` 那条正则的六个捕获组是
`"…"`、`“…”`、`«…»`、`「…」`、`『…』`、`＂…＂`，命中哪一个都包成 `<q>`，
`.mes_text q` 再统一上 `--SmartThemeQuoteColor`（`[ST] style.css:554`）。
没有任何开关能分开它们——§121 记的就是这条规则整条搬过来的过程。

**Iris 的默认是三对：`“…”`、`"…"`、`«…»`。** 也就是**说话用的**那三对。
`「…」` 与 `『…』` 在中文排版里更多是**术语号、书名号、强调号**，主人给的例子是一句
普通的正文：

> 从「资格」转为「职责」。

这里没有一个字是谁说的。按上游的规则，这句话里有两段「对白」会被染成引号色；
一屏正文里这样的词往往不止两个，于是**颜色不再指向对白，只是把页面弄花**。
`＂…＂`（全角直引号）跟着走同一侧，因为它与前两者出现在同一批文本里，
而真正的说话场合几乎都写成 `“…”`。

**裁定（主人，2026-09-20）：这是一条有意的、写下来的偏离**，不是向上游看齐的失手。
兼容地板仍然可达：设置里给了开关，选「与 SillyTavern 一致（六种）」就回到上游那一套。
...
| --- | --- | --- |
| 对话引号 | `"…"`、`“…”`、`«…»` | **是** |
| 与 SillyTavern 一致（六种） | 上面三对，加 `「…」`、`『…』`、`＂…＂` | 否 |

存的地方与它的邻居一样：`localStorage` 的 `iris.quoteScope`，
和 `iris.bodyTag`、`iris.language` 同一个家、同一套 safeRead/safeWrite 容忍度
（`app/quote-scope.ts`）。它只改**显示**，不改存的字、不改发出去的字。
**改了立刻生效**：偏好是一个外部 store，消息行用 `useSyncExternalStore` 订阅它，
并且把它放进了上色 effect 的依赖表——屏幕上已经在的那些消息会当场重上一遍色。
依赖表里少这一项，开关就只对**下一条**消息有效，而这正是 `quote-scope.test.ts`
用一棵挂载起来的消息树去钉的那条。

**规则本身一个字没改。** 代码分支（`<style>`、```` ``` ````、`~~~`、
```` `` ````、`` ` ``）、标签内引号的 U+FFFE 遮罩、**没有 `s` 标志**、
引号留在 `<q>` 元素里面、扫描跑在渲染之后的那一遍 DOM 上——全部是 §121 记下的样子。
变的只有「拿哪几个捕获组去跟文本对」：`quotedDialogueRuns(text, scope)` 的第二个参数，
默认 `'dialogue'`。

### 牙齿

每一条都先掰断代码看它红，再改回来看它绿。

| 断言 | 在哪里 | 掰断它的改动 | 实测 |
| --- | --- | --- | --- |
| 默认包三对说话引号，引号在元素里面 | `quoted-dialogue.test.ts`「the three speech kinds wrap by default」 | `DIALOGUE_PAIRS` 去掉 `(«.*?»)` | 红 → 绿 |
| `「…」`/`『…』`/`＂…＂` 默认**不**包 | 同上「the three bracket kinds are terms, not dialogue」 | `QUOTE_SCOPE_DEFAULT` 改成 `'upstream'` | 红 → 绿 |
| `'upstream'` 下六对都包 | 同上，后半 | `quoteScan` 忽略 `scope`，永远只用 `DIALOGUE_PAIRS` | 红 → 绿 |
| 一句里对白与术语并存时，默认 1 段、`'upstream'` 2 段 | 同上「a line of speech and a bracketed term in one sentence」 | 同上两条任一 | 红 → 绿 |
| scope 传得到渲染后那一遍 DOM | 同上「the scope reaches the DOM pass」 | `markQuotedDialogue` 调 `quotedDialogueRuns(text)` 时把 `scope` 丢掉 | 红 → 绿 |
| 偏好写得进设备存储、通知订阅者、损坏值退回默认 | `quote-scope.test.ts`「the preference round-trips…」 | `setQuoteScope` 不再 `safeWrite` | 红 → 绿 |
| 开关一翻，**屏幕上已有的**消息当场改色 | 同上「a message on screen re-colours itself when the setting flips」 | 上色 effect 的依赖表里去掉 `quoteScope` | 红 → 绿 |

最后一条是这一节唯一需要把消息行真正**挂载起来**才能问的：依赖表是那种在源码里
读着对、在页面上什么都不做的东西，服务端渲染又根本不跑 effect。所以
`quote-scope-harness.tsx` 用 vite 把真的 `Message` 挂进 jsdom，翻开关，数 `<q>`。

### 一次稳定性测量（不是这条偏离的断言，是顺手的读数）

`qa/quote-stability.mjs`：在活宿主上开**黑兽**（带脚本的卡，主体）与 **Assistant**
（无卡无脚本，对照），在开启后 2 s / 10 s / 30 s 各读一次
`document.querySelectorAll('.iris-msg__text q').length`，再做三次「不动消息依赖」的重渲染
（收起并展开状态栏、改视口宽度再改回来、打开并关闭提示词面板）后各读一次。
2026-09-20 实测（第二趟，`QUOTE_TAG=proved`）：黑兽 19 / 19 / 19 / 19 / 19 / 19 / 19，
Assistant 2 / 2 / 2 / 2 / 2 / 2 / 2；
引号色 `rgb(138, 100, 32)`，正文墨 `rgb(35, 32, 31)`，两者不同。**没有掉色。**
三次重渲染里只有**提示词面板**那次是**能被证明发生过**的——它是 `ChatPane` 自己的
`explaining` state，`Message` 既没被 memo 也没有 key 变化，面板出现在 DOM 里就是这次提交
发生过的证据；另外两次改的是壳，React 在输出相同的时候什么都不写，所以
「行重渲染了而颜色还在」与「行根本没重渲染」在读数上同形。

**同一脚本不同趟之间，黑兽的绝对数会变**（三趟实测 33 / 19 / 19，`.iris-msg__text`
行数 7 / 6 / 5、界面槽位 7 / 5 / 5）：这张卡的界面 claim 是异步落定的，落定得多，
散文里剩下的字就少，能上色的对白也就少。**趟内的读数从不动**（三趟都是一条平线），
而趟内才是「重渲染会不会掉色」这个问题的单位——跨趟的差是另一件事，与这条偏离无关。

### What would overturn this

(a) **主人改口**：这条是裁定，不是测量结论；改了默认就改，账本上换一节。
(b) **上游自己分开这六对**（比如给 `power_user` 加一个开关）：那时 `'upstream'` 这一档
要跟着上游的新语义重新定义，而不是继续指「2026-09-20 的那六对」。
(c) **语料测出相反的事**：如果一批真实中文卡里 `「…」` 主要用来写对白（而不是术语），
...
## 128. 分支树：边栏下半的树图、楼层上的「⑂N」、「分支」与「转成分支」

**Kind:** Upgrade（主人 2026-09-25 的分支树需求与同日的摆放裁定、「点节点直接切换」裁定）。宿主那半——持久行 id、
`iris.branchAt`、只读的 `chat.tree`——是 host 账本 §103。编号待协调者重排。

**上游怎么做。** SillyTavern 的分支在消息的扩展菜单里（`bookmarks.js` 的「Create branch」），切出一个新聊天文件并
打开它；父聊天那一楼的 `extra.branches` 记下子聊天名，界面上这一楼没有任何标记，也没有一张图能看出谁从哪一楼走开。
读法（swipe）只能在最后一楼左右翻。

**Iris 的做法。**

- **摆放。** 右侧变量栏上下分开：上面是原来的变量，下面是「分支」树图，中间一条可以拖的分隔线（方向键也能挪，高度按设备
  记住，读写都包在 try/catch 里）。窗口窄到变量栏让出这一列（1440px 以下，或者栏被收起）时，页眉出现「⑂ 分支」，把同一张图
  作为浮层打开；在浮层里点了节点，浮层就关上。页眉按钮什么时候出现，由 CSS 决定（`:has(.iris-aside[data-iris-aside='shut'])`），
  所以正好是边栏那份不在屏幕上的时候。
- **树图。** 竖着的 git 图：0 楼在最上面，每个对话（也就是每个 SillyTavern 聊天文件）占一列。没有分叉的连续楼层折成一段「⋯ N 层」。
  当前对话那一列和读者所在的楼层用强调色画出；读者这条线经过的祖先列，只有它真正读过的那一截是强调色。
  **点节点直接切到那个对话的那一楼**，中间没有预览：一行楼号点了，去的是读者这条线上的那一楼；标题点了，去的是那个对话
  的最新一楼（最早的版本只有一个按钮，显示分支的名字，点了却去父对话，已改掉）。读法数只是节点上的「×N」，不占列。
  一个分支若是在父对话自己也是抄来的那一楼上切的，连线从真正画着那一楼的祖先列出发。
- **楼层徽标「⑂N」。** 有别的对话从这一楼走下去，这一楼的边注里就有一个「⑂N」，点开列出来，选一个就切过去（带上这一楼）。
  它数的不只是本对话自己的子分支：分支自己切出来的那一楼上，「回到父对话接着走」也算一条路，同一楼切出的兄弟分支也算。
- **「分支」与「转成分支」。** 每一楼的操作行里多了「分支」（`chat.branch`，保留到这一楼）。有多个读法的回复，读法条旁边有
  「⑂ 转成分支」，用屏幕上正在显示的那个读法作为 `swipeId`。两者都会打开新分支，并把切开的那一楼滚到屏幕上。
- **跳到某一楼。** 窗口外的楼层先把挂载窗口放宽，行存在之后再落位，落位的那一行淡淡亮一下。落位后 2.5 s 内，上方的布局变化
  会再落位一次（读者滚动、点击、打字就放手）。这是验收时实测补上的：一张卡片对话的分支切在 1 楼，落位以后开场白的界面
  帧又长高了约 2200px，把那一楼顶出了屏幕。

**实现分层。** 画什么由纯函数 `app/tree-map.ts` 决定（`layoutTree`、`branchesAt`），`TreeMap.tsx` 只负责渲染。store 的一片放在
`client/branch-tree.ts`（`tree`、`floorJump`、`treeFocus`，以及 `loadTree` / `branchChat` / `jumpToFloor` / `settleFloorJump`），
`store.ts` 只是把它展开进来。树在对话切换、楼层数变化、对话列表变化时重取；给已经离开的对话的旧答复会被丢弃。
假客户端的 `chat.branch` 现在是真实现（它原来的拒绝注释写着「谁做了分支界面就把这一臂补上」），`chat.tree` 也有了。

**测量。** `tests/tree-map.test.ts` 8 条：一个家族里同时有根、普通分支、分支的分支、读法分支（它那一列从分叉楼本身开始）、
还没写任何东西的新分支；折叠（120 楼的直线对话只有三行）；焦点落在共享楼层时画在祖先列上；读者路径逐列标记；读法只是计数；
徽标列出的各条路；在「父对话也是抄来的楼层」上切的分支从画着那一楼的列出发（验收里遇到的形状）。
`tests/branch-tree-store.test.ts` 3 条（针对假宿主）：分支后打开新分支并请求把分叉楼滚到屏幕上；跳到另一个对话是直接切换；
给已离开对话的树不会画上去。验收 `qa/branch-tree-acceptance.mjs`（8794，数据目录用拷贝）全部通过，截图在
`qa/results/branch-tree/`。

**何时重开：**(a) 家族大到树图的列放不下（边栏 236px 里超过约 10 列）：那时要做列复用（已结束的列让给后来的分支），或者横向滚动
只给图那一栏。(b) 主人要「预览再切换」：点节点目前是一步到位，按裁定就是这样。(c) 读法也要画成分支：那就得先有「读法的后续」
这个概念，现在读法没有后续。

## 129. 分支管理：树图上删除与重命名，每条分支一种颜色，线越活跃越粗

**Kind:** Upgrade（主人 2026-09-26 的分支管理需求）。宿主那半——删除时子分支改挂、删根提升——是 host 账本 §106。编号待协调者重排。

**上游怎么做。** SillyTavern 没有树图；删聊天在聊天管理列表里一行一行删，分支之间没有关系可言，也没有颜色或粗细。

**Iris 的做法。**

- **删除。** 树图每一列的名字旁有「⋯」（悬停或聚焦时出现；触屏常驻），菜单里「重命名」「删除分支」（根上写「删除对话」）。确认框写出
  这段对话的名字和层数，说明子分支会接到谁身上（删根时说明哪一个会成为新的根），并给一个「同时删除它下面的 N 个子分支」复选框
  （勾上后按钮变成「删除 N 段对话」，发 `subBranches: 'delete'`）。备份保留写在框里。删的是正在读的那段（或勾了连子树、正在读的在子树里）时，
  框里写明会转到哪里，删完转到它的父亲，删根则转到被提升的新根——不再是「列表最上面那一段」。边栏的「删除对话」走同一个 `deleteChat`，
  所以也按父亲转、子分支也改挂。
- **重命名。** 菜单「重命名」、或在名字上双击 / 按 F2，名字就地变成输入框：回车或离开保存、Esc 放弃、空名不发。复用 `chat.rename`，
  边栏、树图、页眉读的都是它答回来的列表，所以三处同时变。
- **每条分支一种颜色。** 根是主干，画中性的 `--iris-ink-tertiary`；分支从六色调色板取色：石青、青、金（语境表已有的三个 token），
  以及三种混色——石青混梅色的淡调、石青混墨色、青混金。都由主题 token 推出（`tree-map.css` 的 `--iris-lane-0…5`），雪、墨、宣各自在
  自己的纸上重画。强调色不在调色板里：它是读者这条路径，画得比所在的那列略宽，盖在列色上面。取色 `laneHues`：按对话 id 的哈希
  （FNV-1a）取槽位，同一家族内槽位被占则顺延到下一个空槽，按哈希值（再按 id）依次分配，与列表怎么排无关；家族里分支不超过六条时互不相同。
  边栏分支行的「↳」戴同一个颜色——两边用同一个函数、同一份 id 与父链接各自算，不互相询问。
- **线越活跃越粗。** `laneActivity`：`score = 0.7 × min(1, log2(1+自己的楼层)/log2(65)) + 0.3 × 0.5^(距家族最新活动的天数/7)`，
  线宽 = 1.5 + 2.5 × score（px，1.5–4）。「自己的楼层」是分叉之后写的（根是全部）。时钟取家族里最新的 `updatedAt` 而不是 `Date.now()`，
  所以同一棵树明天画出来一样，一个月没碰的家族也看得出当时哪条最热闹。悬停在一列上，提示里写着这条线的宽度、活跃度、自己的层数、
  比家族最新早多久，以及这个定义本身。

**顺手修掉的一处。** 树图的楼层列表压在图上面，它的左内边距正好盖住图那一栏：此前图上的点接不到点击、也就不可能有提示。
列表改为 `pointer-events: none`，行本身照旧接指针。

**测量。** `tests/tree-lane-style.test.ts` 8 条：小家族四条分支四种颜色（其中 a 与 s 的哈希槽相同，靠顺延分开）；颜色随对话不随列序或列表序，
左边新加一列不动别人的颜色（除非新来的正好要那个槽）；边栏行与树节点给出同样的颜色，父亲已不在的是主干；超过六条时六色用满才重复；
线宽的端点、上界、对数（8 层是楼层项的一半而不是八分之一）、7 天减半；线宽不随 `Date.now()` 变；删除摘要（层数、改挂到谁、被提升的新根）。
`tests/tree-lane-palette.test.ts` 1 条：从 `tokens.css` 与 `tree-map.css` 在三套主题下算出六色，每色对页面 ≥ 3:1、两两相距 ≥ 30、离强调色 ≥ 60
（第一版的「石青混青」在雪里离石青只有 17，换成「石青混墨」）。`tests/branch-tree-store.test.ts` 新 3 条（假宿主）：删中间分支后孩子挂到根；
删正在读的那段转到父亲而不是列表最上面那段（牙齿：改回「列表第一段」→ 红 1）；`'delete'` 连子树且读者落到被删分支的父亲；重命名进列表、视图与树。

**验收。** `qa/branch-manage-acceptance.mjs`（8804，拷贝的数据目录，全程不生成：页面发出的 RPC 里没有 `chat.send` / `regenerate` 一类）全部通过。
用界面的「分支」按钮做出 P→A(2)→{C(1), B(2)→D(2)}、P→E(24)：六列、主干一列、五种分支色；根（27 层自己的）3.65px，新分支 2.25px；
边栏「↳」与列的色块计算值相同；墨下同一列重画成另一色。就地重命名后页眉、边栏、树图、`chat.list` 都是新名字。删中间的 A：框里写
「有 3 层」「接到『爱衣·验收』上」，删后 C 在 1、B 在 2、D 仍属 B，没有孤列。读着 B 删 B：转到 P，D 挂到 P 的 2。读着 P 删根：C 被提升，
读者到 C，D 与 E 从 C 的 1 层分出，E 拥有原根的大部分楼层，成为最粗的一列（3.58px）。截图在 `qa/results/branch-manage/`。

**何时重开：**(a) 颜色要在删掉兄弟分支之后也绝对不变：现在一个分支的颜色只在「比它哈希小、又要同一槽位」的分支出现或消失时才会变，
验收里删掉 A、B、P 之后 E 由第 3 色回到它本来的第 1 色；要绝对不变就得把分配记下来（每家族一份），而不是每次算。
(b) ~~「⑂N」楼层徽标的列表里也要能删~~——已接上，见 §134：列表下的「删除」组打开同一个 `BranchDeleteDialog`，走同一个 `deleteChat`。(c) 家族超过六条分支：颜色开始重复，届时考虑按深度或按父亲分组取色。
...
## 133. 分支段总结：树图上的悬停卡片、「总结这一段」、「总结所有分支段」

**Kind:** Upgrade（主人 2026-09-26：每个分支段都有模型写的总结，光标悬停时显示）。宿主那半是 host §108。编号待协调者重排。

**上游怎么做。** 没有树图，也没有按段的总结（host §108 说明与 Summarize 扩展的关系）。

**Iris 的做法。**

- **悬停。** 树图上每段都能悬停：折叠行「⋯ 29 层」（卡片列出这一行所在的每一列的段——两列并行穿过同一折叠行时两段都列）；
  列上两个点之间的线段（`SegmentHits`：每段一条透明的宽描边，画在点的下面，所以点的点击照旧；一层的段是零长路径，圆端帽在点周围
  形成一圈；悬停时以 accent 淡色显出段的范围，比列本身的活跃度提示目标窄，列边缘仍显示线宽说明）。卡片挂在所在行下面，显示
  楼层范围、总结、生成时间与模型；过期时写「这段内容在总结之后改动过」并给「重新总结」；没有总结时写「这一段还没有总结」并给
  「总结这一段」。折叠行原来的原生提示（跳到哪一层）移进卡片底部，免得两层提示叠在一起。
- **键盘。** 折叠行的按钮、楼层号、列名在键盘聚焦（`:focus-visible`）时打开所属段的卡片；卡片渲染在触发它的那一行里、紧跟在
  触发元素后面，所以 Tab 直接走进卡片的按钮；Escape 关闭。指针或焦点离开 180 ms 后关闭，指针能从线段移进卡片点按钮。
- **只在按下时花钱。** 悬停只读（`chat.segmentSummaries`，在树重读、或打开的对话的 view 变化——编辑、换读法——之后 120 ms 读一次）；
  请求只由卡片的按钮或「总结所有分支段」发出。
- **「总结所有分支段」** 在树图标题栏（边栏）与窄窗口的树图浮层顶部。先确认，写明要总结几段、其中几段是重新总结（已新鲜的段不再
  花钱）；确认后逐段进行，一次一个请求，按钮变成「停止（k/N）」。停止：不再开始下一段；没有回合在流式时还调 `chat.abort` 中止
  进行中的那一个（同一开关会停掉读者的回复，所以回合在流式时只停在当前段之后）。某段失败，错误留在它的卡片上，其余照常进行。
- **用量页。** 新份额「其中分支段总结 N 次」，与卡脚本、压缩摘要、写插件并列；每段对话的小计列也有。
- 主题令牌与 `iris-*` 类名；中英文案（`seg*` 键）。

**测量。**
`apps/iris-web/tests/segment-summary.test.ts` 6 条：每段在图上都有位置、在自己那一列；折叠行代表它所在每一列的段（含两列并行的
形状）；分支抄来的前缀楼层属于根的段；「总结所有」只数缺失与过期；切片：读不发请求、一次只有一个请求在飞、`lane` 随请求发出；
取消后不再开始新请求并中止进行中的那一个。
`apps/iris-web/tests/segment-summary-mount.test.ts`（真 `TreeMap`，jsdom）：挂载只读不写；聚焦折叠行显示已存总结、时间与模型、
`aria-describedby` 指向卡片、新鲜总结不给按钮；Escape 关闭；悬停列上的段给「总结这一段」，点击发出恰好那一段
`{ chatId, fromFloor: 31, toFloor: 40, lane }`，新总结出现在开着的卡片里；「总结所有」先确认、写对段数、确认前不发请求。
牙齿：切片不发 `lane` → mount 与切片各红一条。
真浏览器 `qa/segment-summary-acceptance.mjs`（8806，拷贝数据目录，本地 mock provider）全部通过：在黑兽上用界面「分支」从第 20 层
建一条分支并写两层；真实鼠标悬停「⋯ 19 层」显示「还没有总结」且悬停不发请求；「总结这一段」一次请求、卡片显示总结与时间模型；
「总结所有分支段」确认框写「将总结 4 段」、确认前不发、确认后恰好 4 次、之后按钮禁用；悬停新分支 21–22 的线段与根的「⋯ 7 层」
各显示自己的总结；三段对话读到同一份 5 条的列表、共同前缀只一条且在根上；聚焦折叠行也打开卡片；编辑新分支第 22 层后卡片过期并给
「重新总结」，重新总结后新鲜且来自改写后的文字；用量份额 6 次 = 请求 6 次；新分支那段的请求体只有「此前」一行、两层与指令、
无 system；页面发出的 `chat.summarizeSegment` 次数等于按钮次数；宿主的每个请求都到了本地 mock。
截图在 `qa/results/segment-summary/`（01 未总结、02 已总结、03 确认框、04 列上的段、05 过期、06 重新总结、07/08 墨）。

**何时重开：**(a) 回合流式时按「停止」只停在当前段之后：若主人要立刻中止，需要一个只中止旁路调用、不中止回合的 RPC。
(b) 卡片宽度跟着边栏（约 200 px）：总结若常超过四句，考虑限制高度加滚动。

## 134. 比较变量：树图上挑两层，边栏上半看它们的变量差在哪里

**Kind:** Upgrade（主人 2026-09-26 的需求：分支树上任选两个节点——同一条线的不同楼层，或不同分支的楼层——看变量差异，主要是 MVU 的 `stat_data`）。宿主一侧是新的只读 RPC `chat.variablesDiff`，上游没有对应物，所以没有另开 host 账本条目，写在这里。编号待协调者重排。

**上游怎么做。** SillyTavern 没有树图，也没有跨楼层、跨聊天的变量对比；要比只能分别打开两段对话、各自翻变量表。

**Iris 的做法。**

- **宿主：`chat.variablesDiff({ a: { chatId, floor, swipe? }, b: { … }, tables? })`**，只读。每一侧由 `ChatEntry.floorSnapshot`
  读：助手楼层取所点读法（不给 `swipe` 就取选中的那个）那个候选**最新一次**写入的 `iris/variables`，过一遍清理记录（被清理过的标
  `pruned`）——与 `floorVariables` 同一份记录、同一条「取最后一次」的规则，只是读法可以点名。用户楼层先读文件自己带的
  `chat[i].variables[0]`（经 `iris/st-meta` 恢复，按导出同样的方式套用行级清理），没有就读它所在回合的表，并标 `source: 'turn'`
  ——这就是卡片读这一层时拿到的那份（host 账本 §14 的残差）。没有这层、没有这个读法、这一层从没写过表，分别答
  `missing: 'no-floor' | 'no-swipe' | 'no-table'`，不拒绝；只有对话本身不存在才是 not-found。
- **差异本身在契约里**（`@iris/protocol` 的 `variables-diff.ts`，与 `digests.ts` 同样的理由：宿主和假宿主都要算，两份实现会在最要紧的
  地方悄悄分叉）：对象按自有键比、与键序无关；只一侧有的键整棵子树算一条 `added` / `removed`；数组**按位置**比，路径经过下标的每条都带
  `index` 注；MVU `[值, 说明]` 对在两侧说明相同时按值比，报在这一对自己的路径上并带 `mvu` 注（`好感度: 30 → 35`，不是
  `好感度/0`），说明变了就退回按位置；种类变了（对象↔数组、数字↔字符串、值↔null）是整条路径一条 `changed` 带 `type` 注；键用
  `hasOwnProperty` 问，`__proto__`、`constructor` 这类键就是普通的键。缺失的一侧当空表比。回答里还有计数与 `identical`；
  `tables: true` 时带上两张表本身（页面「只看变化」关掉时要画不变的行）。
- **树图：「比较变量」开关**在边栏下半「分支」那一行。开着时点树图上的点、楼层号、分支名、折叠段，都是**挑**而不是跳：先 A 后 B，
  两个都有了再点就从 A 重来；读者留在原处。挑中的点套一个圈，楼层号或分支名前有 A / B 字标（A 墨色、B 强调色）。任何时候按住
  Shift 点一层也是挑（顺带开模式）；Esc 退出（输入框、菜单、对话框先接到 Esc 时不算）。窄窗口的树图浮层不参与挑选——那里没有地方放比较。
- **边栏上半换成比较**，关掉（✕ 或 Esc）变量树原样回来。头上两行：`A 分支名 #N`、`B 分支名 #M`，楼层号是可以改的框（回车或离开生效，
  按那段对话的层数夹住——折进「N 层」里的楼层就这样挑），多读法的楼层写「第 n 个读法」；右边交换 ⇅ 与关闭 ✕。下面是与变量树同一套
  工具与画法：按名称筛选、「只看变化」（这里**默认开**）、`+/~/−` 计数；树画的是 B 的表，只在 B 有的标「新」，改了的标「改」并且
  **就地**写「A 的值 → B 的值」（短的用改动的琥珀色，长的换行、用正文墨色）；只在 A 有的列在「只在 A 中」下面，带完整路径。
  缺表、读的是回合表、被清理过、有按位置比的，各有一行说明。主机路径与树的路径在 `state-panel.ts` 的 `compareMarks` 里对齐：
  下标从 0 变成树上的从 1；纯标量列表在树上是一行，所以里面的位置差异记在整行上；装着对象的 MVU 对在树上是两行的分支，经过值那一半
  的路径补上树的 `1`。
- **楼层「⑂N」列表**下面加了两组：「比较变量：」（这一层对那段对话的同一层）与「删除」（打开树图同一个 `BranchDeleteDialog`，走同一个
  `deleteChat(chatId, { subBranches })`——§129 何时重开 (b) 的那几行）。原来的每一行照旧直接跳过去。

**测量。** `packages/iris-protocol/tests/variables-diff.test.ts` 12 条：相同输入（含键序不同）、嵌套、整棵子树增删、按位置（插在前面
就是后面每个位置都变）、数组变短、MVU 按值、MVU 说明变了退回按位置、装对象的 MVU、种类变化五种、缺失一侧、`__proto__` /
`constructor` / `hasOwnProperty` 键、空数组与 NaN。`packages/iris-app-service/tests/variables-diff.test.ts` 4 条（`createTestService`，
根对话三层写了表，`chat.branch` 在 2 分出后再写自己的第 4 层）：同一段对话两层；主干 #4 对分支 #4 各读各的、分出时抄过去的 #2 两侧相同；
`no-floor` / `no-swipe` / 用户楼层读回合表 / 未知对话 not-found；点名的读法（重生成后切回读法 0，比读法 1）；`tables` 只在要时才带。
`packages/iris-client-fake/tests/variables-diff.test.ts` 2 条（假宿主种子里 络络 的四张表）。`apps/iris-web/tests/variable-compare.test.ts`
4 条：挑选顺序；`compareMarks` 的三种路径对齐；只在 A 的路径与缺 B；store 在 B 被改了之后把晚到的旧回答丢掉。
`apps/iris-web/tests/variable-compare-mount.test.ts` 2 条（jsdom 里挂真的 `StatePanel` 与 `Message`）：开模式、点 A 点 B、不换对话、
地图上 A/B 字标、计数 `+2 ~2 −0`、`30 → 35`、纯列表一行、「只看变化」开关、交换后 `+0 ~2 −2`、Esc 退出、Shift 点、✕、模式外点击照旧跳；
「⑂N → 比较变量：」两侧都是这一层、「⑂N → 删除」打开删除框并删掉。牙齿（每条改坏一次看它红、再改回）：宿主总读选中读法 → 红 2；
去掉 MVU 规则 → 红 2；`hasOwnProperty` 换成 `in` → 红 1；树路径不加 1 → 红 2；不丢旧回答 → 红 1；模式里点击仍然跳 → 红 1；
去掉 Esc → 红 1；「删除」不开框 → 红 1。顺手：`previewValue` 把纯标量列表写成 `a, b`（树上本来就这么画），原来是 `String` 出来的 `a,b`。

**验收。** `qa/variable-compare-acceptance.mjs`（8807，拷贝的数据目录，全程不生成）全部通过，读的是拷贝里的 黑兽（33 层）与它已有的
「黑兽 - Branch #1」（30 层分出）。#0 对 #30：页面 `+0 ~0 −3` 与宿主对同一对的回答相同；#30 这层从没写过表（`no-table`），页面写明
「B 在这里没有变量：这一层从没写过变量」，只在 A 中列出 `initialized_lorebooks`、`stat_data`、`schema`。主干 #32 对分支 #32（两边都是
第 2 个读法）：`+0 ~4 −0`，同宿主；四处改动是 `stat_data/世界/时间`、`世界/事态`、`女主角/塞蕾斯汀·卢库勒斯/心理活动`、`…/与主角`，
就地写着前后两个值；「只看变化」关掉后 8 行变 84 行；交换计数镜像。读者始终在主干上；Esc 退出后变量树回来；30 层「⑂1 → 比较变量：」
两侧都是 #30（两边都没有表，两行说明）。页面发出的 RPC 里没有生成、没有改聊天；卡自己的帧在打开时照常发了 `script.setVariables` /
`script.setExtensionSettings`，那是 黑兽 的 MVU 脚本，不是比较发的。截图在 `qa/results/variable-compare/`（雪、墨各一张）。
