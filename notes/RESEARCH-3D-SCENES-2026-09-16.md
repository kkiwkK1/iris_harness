...
## 7. 想法记录（2026-09-16，owner 裁定：只记想法，代码层面不动）

**定位**：当前模型做 3D 有用但达不到要求（精细度差、动辄数小时），发展很快；现在做的是为将来的扩展提前准备**基建**。筛选原则：只做在任何模型到来之前都不会作废的东西，模型一换就得重写的东西不做。

现在值得做的：
1. 场景描述契约——版本化、可校验（层级树、局部坐标、命令式增量），进协议与契约包；弱模型时期由手写夹具喂它。
2. 场景增量作为变量结算的一种写入，走已落地的 writer 注册表；swipe / regenerate / continue 自动正确；持久化落在 ST 也能读的扩展字段里。
3. `iris-scene` 渲染插件骨架：`client.js` 在消息帧里起 three.js（jsdelivr 已在白名单），按描述渲染、增量更新。
4. 资产来源策略先定死再放开：默认不开远端，走同源资产面或用户显式装的资产包。
5. P0 探针：沙盒帧内 three.js 的内存、首帧、多帧并存的资源账。
6. U4 钩子实现（`afterReplyText`）提优先级——切场景块的位置。

现在不做的：针对某模型输出习惯的提示词与解析器；几何生成本身；世界模型/视频式生成；契约稳定前的编辑器 UI。

要先下的判断：场景归属是「每条消息一个场景」还是「整个聊天一个持续演化的世界」（倾向后者）。兼容义务另算：ST 的 VRM / Live2D 形象扩展走 ST 扩展兼容面，是「形象」不是「场景」。

**相关想法（同日）——上游第三方插件的更新**：内置 TH / MVU 是重实现，「更新」= 兼容面跟踪。三条路：显示兼容目标（V1）、上游面差分量具（V2）——两项已派工，见 `INFRA-TASKS-2026-09-16-upstream-tracking.md`；第三条「让上游 TH 经 ST 扩展兼容面直接跑，更新走 U1 的事务」需先做可行性探针，暂记。内置实现与上游直跑预期是并存（地板 + 可选提供者），不是替换。
