...
> 状态：已归档（2026-09-16）。这是 2026-09-11 起草、只存在于分支 `dev/feat-extension-system` 的第三方扩展系统契约（扩展 = `cordis.yml` 一行 + 一个 npm 包）。它被 2026-09-13 合入的系统插件平台（#88，`docs/SYSTEM-PLUGINS.md`、`docs/INFRASTRUCTURE-INTERFACES.md`）取代：活下来的是「同进程同权限、靠来源而不靠沙盒」的信任判断，没有被采用的是按 `cordis.yml` 行装载扩展的机制。原路径 `docs/EXTENSIONS.md`；原分支于同日删除。正文原样保留，其中的 `path:line` 引用属于该分支的提交 `589585e`，不是 `main`。
...
The constitution for everything below is `notes/archive/EXTENSION-SYSTEM-RULING.md`
...
Against the Ruling's four completion checks (`notes/archive/EXTENSION-SYSTEM-RULING.md:47`):
