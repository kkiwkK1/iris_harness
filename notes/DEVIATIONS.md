...
5. **`openai_max_context` 成为 `GenerationSettings.contextWindow`（预设域），
   `max_context_unlocked` 成为 `contextUnlocked`，两者同行。**
   装配预算随切换走（实测真预设：4095 / 655 350 / 1 000 000 / 2 000 000）。
   上游单一空间里这自然发生；本侧不把窗口写死在组合里，否则为 4 095 调的
   预设会安静地裁掉对话的另一头。
   窗口会被夹到「模型已知能接受的长度」，除非 `contextUnlocked`。`Math.min`
   这条机制抄的是上游**具名提供方**分支；但本侧只有一种线路（base URL +
   模型名），结构上永远落在上游的 CUSTOM 分支，而那一支把上限设成
   `unlocked_max`（2 000 000）且不查任何模型表——实测用户自己的装机正是
   `chat_completion_source: 'custom'` 指向 `api.deepseek.com`，**上游在这里
   同样不会夹**。所以这是刻意的改进而不是对齐，`contextUnlocked` 就是把
   上游这条线路的原样答案交还回去。另外本侧在**每次读取**时夹，不回写用户
   存的数字。模型窗口的来源（提供方 `/models` 的字段 / 内置表 / 查不到就
   不夹）与全部实测见 `notes/packages/iris-app-service/DEVIATIONS.md` §44。
...
14. **设置抽屉在"更多参数"里补了上下文窗口、解锁开关与推理力度三个读数**，
    因为预设切换会一次改写它们——切换改变的东西若不可见，等于安静地改。
    解锁开关（`contextUnlocked`）紧贴窗口滑杆，因为二者是同一个决定的两半：
    不开时窗口会被夹到模型上限（见上文第 5 条）。
