...
> 状态：记录。测量于 2026-09-07，对象 语料卡（`测试用卡/`）的渲染前形状，经产品自己的 `decodeCardPng` / `normalizeCard` / `extractScripts` 读。记录不随代码更新；Iris 侧的现状以 `docs/` 为准（索引见 `notes/README.md`）。
...
## 一之一、脚本按钮普查的历次读数 —— **这些数是测量,不是不变量**

语料是操作者自己在用的 ST 安装:他每导一张卡、每玩一局,这些数就动一次,**动了不是回归**。
所以它们记在这里,不写进断言。要现值就跑 `npm run census:card-scripts`(只打印,不判决)。

| 读数日 | 卡(解出) | 脚本 | 带按钮的脚本 | 按钮 | `visible: false` | 按钮组关掉的 |
|---|---|---|---|---|---|---|
| 2026-09-01 | 19 | 47 | 18 | 89 | 58 | 1 |
| 2026-09-20 | 20 | 49 | 19 | 95 | 62 | 1 |

2026-09-01 那一行原来钉在 `packages/iris-app-service/tests/script-variables.test.ts` 的
`assert.deepEqual` 里,**连它自己的失败信息都写着「这是测量值,不是不变量」**。结果操作者一
导卡就红,落地闸门每次都被它挡,而读者被一句「先看卡库再看解析器」打发走 —— 一条要人忽略
自己的测试,不是测试。2026-09-20 改掉:断言只留**形状**(按钮的 `name`/`visible` 类型、脚本
id 非空且卡内唯一)、**解析值与原始块的一致**(`script.buttons` 的条数必须等于卡里
`button.buttons` 自己的条数 —— 少认一种容器形状就在这里红,而且报的是哪张卡哪条脚本)、以及
**下界**(卡 ≥ 15、脚本 ≥ 40、带按钮 ≥ 12、按钮 ≥ 60,外加「比较过的脚本数」本身也有下界,
免得一趟什么都没走到也算过)。比例那条留着:**`visible: false` 仍然是多数**,面板默认值靠它。

同日同理由改的还有两处:

- `packages/iris-mvu/tests/json-patch-spelling.test.ts` —— 「带补丁块的楼」原先按**有没有开标签**
  算。2026-09-20 操作者玩的一局里,模型把自己的指令复述进了正文(「需输出 `<UpdateVariable>`
  的 `<Analysis>` 和 `<JSONPatch>`……当前没触发判定,不必输出」)然后正确地没有输出块,于是
  测试红了,而它打印的那句「有块却读成了静默」两半都不成立。现在按**开标签 + 闭标签成对**算:
  全语料 156 楼有开标签、155 楼成对,差的那一楼正是这句复述;22 楼下划线拼法全部成对,**齿没
  动**(把 `json_?patch` 改回 `jsonpatch`,仍然 21 楼静默)。
- `packages/iris-app-service/tests/chat-search.test.ts` —— `messageCount === 677` 换成与**文件自己
  的楼数**比,加一条 `>= 500` 的下界。那局还在被玩,再回一句就红,而「每一楼都在扫描范围内」
  这条性质本来就该由文件自己当标尺。

> 规矩(2026-09-20 定,写在 `CONTRIBUTING.md` 的 Testing 一节):**语料测试断言不变量或下界,
> 不断言精确条数,也不点名一个活目录里的文件。** 要精确数就留在这份记录和普查脚本里。
...
**③之三:在_会记住历史_的通道上,「某报告消失了」根本不会发生 —— 换成时刻判据。**

③之二 说的是**瞬时**通道(3.2 秒的 toast):**缺席什么都不证明**,所以要留痕。
这一条是同一件事的另一头:**持久**通道(宿主报告缓冲、设置抽屉的 Notices 历史)——
**那一行会一直待到缓冲滚掉**,所以**「它不见了」是一个_永远等不到_的判据**。

> **判据要改成:「**末次出现的时刻早于修法落地的时刻**,而且**此后 N 个周期不再新增**」。**

实例(runId 那条修法):

```
「6 script injection(s) are still live」   末次 17:16:44  ×5
首条「cleared 6 … run …:3」                     17:16:56
再跑两个周期 → 上面那条的末次时刻不变
```

**"旧那条还在缓冲里"和"修法没生效"在_一次快照_里长得一样;分开它们的是_时刻_和_增量_,
不是有无。**

##### 把③之二与③之三合起来,是同一条:**判据要配上通道的记忆长度**
...
**同一份缓冲,两种误读,取决于你把它当快照还是当历史 —— 它是历史,永远。**
...
#### 第二个实例:单引号的模式漏掉了双引号(1/1 → 3 卡/12 处)

同一份普查里 `querySelectorAll('…iframe')` 我数成 **1 卡 / 1 处**。44 的全语料普查是
**3 卡 / 12 处**,其中**银麒 10 处用的是双引号** `querySelectorAll("iframe")` —— 我的模式写的是
`querySelectorAll\(\s*['"][^'"]*iframe`,看着两种引号都收,**但同一份表里另外几行的模式
只写了单引号**,而这一族恰好是双引号占多数。

两个实例的形状是同一个,只是方向相反:

| | 模式 | 得到 | 真值 |
|---|---|---|---|
| `onload` | **太宽**(`\.onload\s*=` 什么对象都算) | 24 | ~0(全是 `Image`/`FileReader`) |
| `querySelectorAll iframe` | **太窄**(只认单引号) | 1 | 12 |

> **一个数错了,不会告诉你它是错高了还是错低了。** 太宽给出虚高、太窄给出虚零,而两者
> 在表格里长得一模一样 —— 都是一个数字。所以「这个模式还会命中什么」与
> 「**这个模式会漏掉什么写法**」是两问,必须分开问。

**对代码没有影响**:查询补丁是运行时的,它看的是选择器**字符串的内容**,与源码里用哪种
引号无关。错的只有这份普查,以及它排出来的优先级。

#### 前台验收顺序(嵌套族,按修完两个必空 bug 之后)

**按这个顺序读,任一条不成立就停在那一层**,否则下一层的读数解释不了:

1. **替身的 body 里有没有 `#app`** —— 即 `srcdoc` 到底渲染进去了没有。
   在 `.attr({srcdoc})` 走 `setAttribute` 那条修好之前,这一步必然是空的,而
   **面板挂上、无报错**,看起来像高度问题。
2. **高度链 300/300/300 且可滚动** —— 在 `@scope` 里根用 `:scope`(而不是 `#id`)那条修好
   之前,这一步必然塌成内容高度,同样无报错。
3. **内在尺寸缺口 (1)** —— 卡如果只从样式表给帧定尺寸、或者根本不定,替身(div)没有
   `<iframe>` 那个 300×150 的内在尺寸。这一条是**已知缺口**,不是回归。
...
**口径先写清,这一层决定这些数能说什么**:测量在**顶层页面**上做、用的是**替身复制品**
(320×400),**不是真帧内**。隐藏标签也能测,因为这些判据不需要帧内几何。
**所以下面的数字说的是「替身这套结构在这些输入下会产出什么」,不是「已经在真卡上看到了」。**

| 观测 | 值 |
|---|---|
| 卡的 srcdoc 原文 352 B 经 `<template>` 解析 | head 得 **META + STYLE**;body 得 **`DIV#app`** ✓ |
| 155 B 样式表经 `repointFrameCss`(**refused 0**,根选择器 → `:scope`) | 贴上 ✓ |
| element / html / body 高度 | **400 / 400 / 400** |
| `#app` | **1200**(`min-height:100%`,内容更高) |
| 底色 | `rgb(14,16,40)` = **#0e1028** ✓ |
| 字色 | `rgb(232,226,214)` = **#e8e2d6** ✓ |
| 滚动 | `overflow-y:auto`,可滚 ✓ |
| head 包装 | `display:none` ✓ |

**底色与字色对上 §九.4之二 那条外观判据** —— 那一格要求"不是白底裸 DOM",
这里两个色值都是卡自己 CSS 里的值。

##### 一条承重的替代关系(不只是巧合),需要一个会自己失败的东西

卡在 srcdoc 里写了 **`<html style="height:100%">`**,而 **`<template>` 的解析器把
`<html>`/`<head>`/`<body>` 整个丢掉** —— 我核过这是**我们自己的实现**,不是量法的产物:

- `apps/iris-web/src/sandbox/frame-entry.ts:425` 用 `document.createElement('template')` 解析;
- `apps/iris-web/src/sandbox/nested-frame.ts:58` 的注释已写明该解析器丢弃这三个标签。

> **所以卡自己那条高度声明在我们这条路上_永远不生效_**(真 iframe 会保留它,我们不会),
> 高度链的起点是**我们 html 包装自带的 `height:100%`** 顶上的。
>
> **这不是巧合,是一次替代**:我们丢掉了一条声明,又恰好自己提供了同样的东西。
> **叫它替代而不是巧合,是因为替代有责任方** —— 哪天包装少了这一行,
> **卡的声明救不回来,因为它已经被丢掉了**。

**按 §七「被行为依赖的测量前提要配一个会自己失败的东西」:注释挡不住这个,注释不会红。**
**这一条要一个断言**:替身的 html 包装**必须**带 `height:100%`,断言消息里写明
"卡自己的 `<html style>` 被 template 丢弃,没有别的东西能提供它"。

#### 装配测试(7b)—— 比顶层复制品重一档

**装配**:`sandbox="allow-scripts"` 的 srcdoc 帧、带**我们真实的 CSP**(含 `frame-src 'none'`)、
加载**线上构建产物** `members-17473f41d8ae3e14.js` 与 jQuery 3.5.1,调 `virtualiseNestedFrames`,
**输入是卡的原文**:`$('<iframe>').attr({frameborder:'0', srcdoc:'…'}).on('load', …)`。

| 观测 | 值 |
|---|---|
| `tagName` | **IFRAME** |
| `srcdoc` 读回 | ✓ |
| `contentDocument === null` | **false**(即拿得到文档) |
| `querySelectorAll('iframe')` | **找到 1** |
| `#app` 落在 body 包装里 | true |
| 卡 `<style>` 被重写并 `@scope` 包住 | true |
| 合成 `load` 打到 `.on('load')` | **1 次** |
| note 通道 | **空**(没有任何拒绝) |

##### 这组里最锋利的一条:CSP 写着 `frame-src 'none'`,而卡跑通了

**一个真的嵌套 iframe 在这条 CSP 下会被直接拦掉。** 它能跑通,恰恰因为**替身不是帧** ——
所以这组读数不只是"同源问题解决了",它证明的是:
**替身是这张卡在_禁止任何帧_的策略下还能工作的原因。** 反过来说,
**哪天有人为了省事把替身换回真 iframe,这条 CSP 会当场拦死它** —— 那不是回归,是策略在说话。

##### 与我那份普查对得上,而且是逐项对上

- **`querySelectorAll('iframe')` 找到 1** —— 我的全语料普查把这条列为**替身唯一的真风险面**
  (3 卡 / 12 处),这里证明查询路径确实追加了替身。
- **合成 `load` 打到 `.on('load')` 1 次** —— 正是我把帧 `onload` 从 **0 更正为 1** 的那一处
  (V1.5.4_,`<iframe` 在 495 字符之前)。**全语料只有这一处,而这组测试跑的就是它的调用形状。**
- **`tagName` = IFRAME** —— 我量到卡**从不检查类型**(`tagName`/`instanceof`/`matches` 全 0),
  所以这一项是余量而非必需;**但它让 `querySelectorAll('iframe')` 那条自然成立**,
  两者是同一个决定的两面。

##### 口径:排掉了两种失效,仍不等于那一眼

**排掉的**:①「源码 ≠ 部署」—— 用的是线上构建产物;②「夹具 ≠ 真卡」—— 输入是卡原文。
加上真不透明源,**机制这一层是端到端的**。

> **仍未验的是_几何与可见_**:装配测试不把它放上屏幕。
> **状态:机制端到端已验(装配测试),待人眼。**
>
> 而这正是那条口径要防的误读 —— **上面两张表全绿,不构成"这一族过了"**;
> 前台剩余仍然只有那一句:**面板在屏幕上且能点。**
...
弹窗关闭 ✓ / 键位置 ✓ —— **全部子项 ✓**(见第五轮);
**报告算术 ✓**(第六轮:`trimmed 21 floor(s) and 21 user row(s)` = 事前预测的 42)。**
...
| ⑦ | **点暗区**(scrim 自身 `mousedown`)→ 立关 | **✓**(8789,bundle `index-CpK9g7Qj`) |
| ⑦ | 点暗区后重载再 open → offer 再来 | **✓** |

**⑦ 补齐了规格里的第二种手势。** 至此 **`Esc` 与「点外关闭」两种手势都验过,且两种都等于
「推迟」而不是「拒绝」** —— 正是那句底注
(`Closing this without choosing asks again next time — it does not decline.`)要求的行为。
**两种手势都要单独验**:实现很容易只给弹窗根节点挂 `Esc`、而把 scrim 的点击接成
"当作 never",两者在"弹窗关掉了"这一层看起来一样。

##### 口径:合成事件的形状本身会伪造一个缺陷

第一次派的合成 `mousedown` **没带 `view`**,**React 的处理器不响** —— 看起来像"点暗区无效"。
带上 `view` 与坐标后立关。**所以这条读数写明:用带 `view`/坐标的事件。**

> **这是本格第三次「仪器形状被读成产品缺陷」**:
> ① 25 秒观察窗对 3.2 秒的 toast → 读成"没发";
> ② 「再发一句」当成询问时机 → 读成"关闭被当成拒绝";
> ③ 合成 `mousedown` 少 `view` → 读成"点暗区无效"。
>
> **三次都不是产品错,是_我们观察的方式_错**,而三次的表象都落在"功能没生效"这一侧。
> **所以每条读数都要带上仪器口径**(观察窗口 / 触发时机 / 事件形状),
> 否则下一个人会去修一个没坏的东西。
...
#### 第六轮:含 user 行的周期 trim(验的是「删除报告少报一半」那条修法,b95d2ff)

真宿主读数:

```
stripped user  613..653  n=21
stripped reply 612..654  n=21
650 snapshot:true
0 / 300 / 610 full
Notices: trimmed 21 floor(s) and 21 user row(s) … nearest intact floor below them is message 610
```

**与 49 事前写下的 42 一致**,而且**旧文本只会说 21** —— 所以这条读数验的不是清理本身,
是**报告的完整性**。

**内部自洽,可以自己核**:user 行是奇数下标,`613..653` 步长 2 = **21**;
reply 行是偶数下标,`612..654` 步长 2 = 22,**扣掉 650(间隔快照)= 21**;
**合计 42**,正是事前那个数。`610` 是保护窗口下界之下最近的完整层,与 Notices 文本一致。

##### 这一格必须读两个东西并**互相比对**,单读任何一个都不够

错的实现有两种,而它们**在单一观察上和正确实现重合**:
...
> - **只看报告**:读到 21 分不出甲和乙 —— 一个是"没做",一个是"做了没说"。
> - **只看磁盘**:看到 user 行被剥,分不出乙和正确 —— 磁盘不告诉你报告说了什么。
> - **两个一起读、并且互相比对**,三种情况才彼此分开。
>
> **所以这一格的判据不是两条独立断言,是一条_关于两者一致性_的断言。**
> 这与本单其他格子不同:多数格子是"看这一处";这一格是"看两处,且它们必须说同一件事"。

**为什么这条值得单列而不是并进第五轮**:第五轮验的是 `never` 那条链,
这一轮验的是**报告文本本身的算术**。**同一个 profile 能跑两轮,但它们证的东西不重叠** ——
第五轮全绿时,这一轮的错法乙**照样全绿**。
