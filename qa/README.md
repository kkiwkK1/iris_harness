...
| `variable-compare-acceptance.mjs` | 树图「比较变量」的验收（web 账本 §134）：自起宿主，端口 **8807**（启动前断言空闲，`IRIS_PORT` 可改），CDP **9375**，数据目录用拷贝（`dataCopy`，退出时和 Chrome 配置一起删掉）。读拷贝里的 黑兽 与它已有的「黑兽 - Branch #1」（30 层分出），**不花钱、比较本身不写**：每一步都是树图或比较头上的点击，页面发出的 RPC 全记下，没有生成、没有改聊天（卡自己的帧在打开时写 MVU 变量，单列报告，不算在比较头上）。依次验：开模式后上半边换成比较；点 #0 为 A、点主干最新层为 B，再用头上的楼层框把 B 改到 #30，页面计数与宿主 `chat.variablesDiff` 的同一对相同，没有变量表的一侧写明原因；主干 #32 对分支 #32（分支名上点），计数同宿主、两侧名字都在头上、改动行就地写「前 → 后」、「只看变化」关掉行数变多、交换后计数镜像（雪、墨各截一张）；Esc 退出、变量树回来；30 层的「⑂1 → 比较变量：」两侧都是 #30。读者始终留在主干上。截图写到 `qa/results/variable-compare/`。硬失败 1 = 有检查未过、2 = 端口/数据目录问题、3 = 硬超时 |
...
| `stable-column-caliper.mjs` | 阅读列不随两翼移动（web 账本 §135）：自起宿主，端口默认 **8805**（启动前断言空闲），CDP **9375**，数据目录用拷贝（`dataCopy`，退出时和 Chrome 配置一起删掉）。**不花钱**。按侧栏标题打开 黑兽（`IRIS_CHAT_TITLE` 可改），在 1920×1047 与 1440×1047 各用界面自己的按钮走四种两翼状态（都收起 / 变量栏开 / 都开 / 侧栏开），每种状态用 `getBoundingClientRect` 量阅读列、最高的卡片帧、全部卡片帧、输入框、页眉标题、「设置」、「收起卡片界面」和回合刻度条，四种状态必须逐项相等；每个卡片帧里（同进程的执行上下文，或 `Target.setAutoAttach` 接上的独立进程帧）装 `resize` 与根宽度计数，阅读列里的帧一次都不许量（卡片舞台上的脚本/浮层帧另记，它们跟舞台走）。另记：滚动条实宽、开关时起了哪些动画（不许有 `width`）、每种状态一张图、每个宽度一张冻结在滑动中途的图（`getAnimations()` 暂停在 45 ms）、`prefers-reduced-motion` 下两块面板不动画、1440 下面板盖在页面上时焦点进出与 Escape。`IRIS_QA_LABEL` 分目录，`IRIS_WEB_DIST` 可指向旧构建做对照（Windows 上要写反斜杠路径，正斜杠会 403）。截图和 `table.json` 写到 `qa/results/stable-column/<LABEL>/`。硬失败 1 = 有检查未过、2 = 端口/数据目录问题、3 = 硬超时 |
