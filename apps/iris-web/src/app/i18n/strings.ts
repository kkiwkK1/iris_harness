...
export const en = {
  ...
  turnNavigationLabel: 'Conversation navigation',
  turnNavigationJump: 'Jump to exchange {n}',
  turnNavigationEntry: 'Exchange {n}',
  ...
  pluginFixFacadeMismatch: 'It was written for a newer Iris. Update Iris, or say another sentence to have it rewritten for this one.',
  ...
  /* Managing a branch from the tree map: the menu on a lane's name, inline rename, and the delete dialog. */
  treeActionsFor: 'Actions for {title}',
  treeRename: 'Rename',
  treeRenameAria: 'New name for {title}',
  treeDelete: 'Delete branch',
  treeDeleteTitle: 'Delete “{title}”?',
  treeDeleteFloors: '“{title}” has {floors} floors. Its file is removed; backups are kept, so a snapshot can bring it back.',
  treeDeleteReattach: 'Its {n} sub-branches stay: they re-attach to “{parent}”, each at the floor it really leaves from.',
  treeDeletePromote: 'This is the root. “{child}”, the branch that leaves it earliest, becomes the new root, and any other sub-branches re-attach to it.',
  treeDeleteCascade: 'Also delete its {n} sub-branches ({total} conversations in all)',
  treeDeleteViewing: 'You are reading this conversation; you will be taken to “{next}”.',
  treeDeleteConfirm: 'Delete',
  treeDeleteConfirmMany: 'Delete {n} conversations',
  /* The tooltip on a lane: what its line weight means (“the more active, the thicker”). */
  treeActivity: '{title}\nLine {width}px, activity {pct}%: {own} floors of its own since the fork, last active {age} before the newest in this family.\nWeight 1.5–4px: 70% its own floors (log scale, full at 64), 30% recency (halves every 7 days).',
  ...
  /* 比较变量: the tree map's compare mode, and the diff it puts in the margin's upper half. */
  compareMode: 'Compare variables',
  compareModeTitle: 'Pick two floors on the map to compare their variables. Shift-click a floor works any time; Esc leaves.',
  comparePickA: 'Click a floor on the map below to pick A.',
  comparePickB: 'Now click a second floor for B.',
  compareAria: 'Variable comparison',
  compareSwap: 'Swap A and B',
  compareClose: 'Close the comparison',
  compareFloorAria: 'Floor of side {side}',
  compareReading: 'reading {n}',
  compareLoading: 'Comparing…',
  compareIdentical: 'The two floors hold the same variables.',
  compareNoChanges: 'Nothing differs under this filter.',
  compareMissing: '{side} has no variables here: {why}.',
  compareMissingFloor: 'that floor does not exist',
  compareMissingSwipe: 'that reading does not exist',
  compareMissingTable: 'nothing was ever written on that floor',
  compareFromTurn: '{side} is a user line; its turn’s table is shown, which is what a card reading that floor is given.',
  comparePruned: '{side}’s table was trimmed by a cleanup; what survived is compared.',
  compareByIndex: 'Lists are compared position by position.',
  compareRemovedHead: 'Only in A',
  compareMarkTitle: 'Side {side} of the comparison',
  compareSummaryAria: '{added} only in B, {changed} changed, {removed} only in A',
  /* The ⑂N list's two grouped actions. */
  forkCompare: 'Compare variables with',
  forkDeleteGroup: 'Delete',
  ...
  networkGrantedNote: 'This card’s frames may load images, styles and fonts from, and fetch, any https host — a card whose whole UI lives on its author’s server needs exactly this. Anything the frame can see can leave over that channel, including the conversation in front of you. Plain http stays refused, and remote code stays on the measured allowlist either way. It stays this way until you turn it off.',
  ...
  networkGrantDialogCancel: 'Keep it off',
  networkGrantDialogConfirm: 'Grant network access',
  ...
  /*
   * `/doctor` (`doctor.ts`). One notice: a count line, then one line per check —
   * glyph, label, what was read, and, when the verdict is not a pass, what to
   * do. The `…Fix` rows are that last clause and are written as instructions;
   * every other row is a reading and says only what was found. A remedy names
   * the place the fix is made (a settings section, an environment variable, a
   * command), because a row that knows the fix and does not say where is the
   * half of a diagnosis that sends the reader searching.
   */
  commandDoctorSummary: 'Check this host, this page and this conversation, and say what to fix',
  commandDoctorFailed: '/doctor could not finish — {reason}',
  doctorHeading: 'Doctor: {ok} fine, {warn} to look at, {fail} to fix',
  doctorRowWithRemedy: '{row} — {remedy}',
  doctorUnreadable: 'could not be read ({reason})',
  doctorLabelHost: 'Host',
  doctorLabelBundle: 'Page build',
  doctorLabelProvider: 'Provider',
  doctorLabelProviderTest: 'Provider test',
  doctorLabelAuthoring: 'Plugin writing',
  doctorLabelConsent: 'Card scripts',
  doctorLabelDataDir: 'Data directory',
  doctorLabelFaults: 'Host faults',
  doctorLabelSandbox: 'Card sandbox',
  doctorLabelPolicy: 'Page policy',
  doctorLabelPlugins: 'System plugins',
  doctorLabelStorage: 'Card storage',
  doctorLabelCorpus: 'SillyTavern directory',
  doctorHostRead: 'Iris {version}, Node {node}, pid {pid}, port {port}',
  doctorHostUnreadable: 'the host did not answer the doctor’s question ({reason}), so the build, data-directory, storage and SillyTavern rows are skipped',
  doctorHostUnreadableFix: 'a host older than this page does not know the question — restart the host on the current build, then reload',
  doctorBundleNotBuilt: 'this page booted from {page}, not a built bundle, so it was not compared',
  doctorBundleHostNone: 'this page runs {page}, and the host serves no built interface',
  doctorBundleHostNoneFix: 'run npm run build in apps/iris-web, then restart the host',
  doctorBundleHostUnreadable: 'this page runs {page}, and the host’s built index names no entry it could read',
  doctorBundleHostUnreadableFix: 'rebuild the interface (npm run build in apps/iris-web)',
  doctorBundleStale: 'this page runs {page}, but the host now serves {host}',
  doctorBundleStaleFix: 'reload the page — it is still running the build from before the host changed, so newer features are missing',
  doctorBundleSame: 'this page runs {entry}, the build the host serves now',
  doctorProviderMissing: 'no provider is in use',
  doctorProviderMissingFix: 'add one under Settings → Connections and press Use; nothing can be generated until then',
  doctorProviderInUse: '{name}, model {model}',
  doctorProviderAnswered: 'answered in {ms} ms',
  doctorProviderRefused: 'did not answer: {reason}',
  doctorProviderRefusedFix: 'open the provider under Settings → Connections and check its address and key',
  doctorAuthoringUnset: 'no model is chosen for writing plugins, so 「Grow a feature」 is off',
  doctorAuthoringOrphan: 'the plugin-writing model {model} names a provider that no longer exists',
  doctorAuthoringUnsetFix: 'pick one on the 「Writes plugins」 row under Settings → Connections',
  doctorAuthoringSet: '{model} on {name}',
  doctorConsentNone: 'this card has no scripts',
  doctorConsentUnasked: 'this card’s {n} script(s) are waiting for your answer, so they have not run',
  doctorConsentAskFix: 'answer the question above the conversation',
  doctorConsentDeclined: 'you declined this card’s {n} script(s), so they do not run',
  doctorConsentDeclinedFix: 'if the card needs them, allow them under Settings → Scripts',
  doctorConsentAllowed: '{n} script(s) allowed; remote network not granted',
  doctorConsentAllowedNet: '{n} script(s) allowed; remote network granted',
  doctorDataDirOk: '{path}, writable, lock held by this host (pid {pid})',
  doctorDataDirNotWritable: 'the host reports {path} as not writable',
  doctorDataDirNotWritableFix: 'fix the directory’s permissions, or point IRIS_DATA_DIR at a writable copy and restart',
  doctorLockMissing: 'host.lock is missing from {path}',
  doctorLockMissingFix: 'restart the host so it takes the lock again — until then a second host could open the same directory',
  doctorLockOther: 'host.lock in {path} names pid {lock}, not this host’s {pid}',
  doctorLockOtherFix: 'another host has opened this directory; stop one of them, because two hosts on one directory overwrite each other’s saves',
  doctorFaultsNone: 'no faults held since the host started',
  doctorFaultsHeld: '{n} fault(s) held; newest ({kind}): {message}',
  doctorFaultsFix: 'Settings → Diagnostics → Host reports lists each one with its cause',
  doctorSandboxOk: 'the manifest and {preset} are served',
  doctorSandboxBroken: 'the sandbox build is not served: {reason}',
  doctorSandboxFix: 'rebuild the interface (npm run build in apps/iris-web) — card scripts and interfaces cannot start without it',
  doctorPolicyOk: 'the shell’s Content-Security-Policy is in this page',
  doctorPolicyMissing: 'this page carries no shell Content-Security-Policy',
  doctorPolicyMissingFix: 'open the interface from the Iris host’s own address rather than a dev server',
  doctorPluginsOk: '{n} installed, none failed',
  doctorPluginsFailed: 'failed: {plugins}',
  doctorPluginsFix: 'Settings → System Plugins shows the failure and a retry',
  doctorStorageRead: '{used} of {limit} used',
  doctorStorageFullFix: 'nearly full, so cards’ next writes will be refused; clear an old card’s storage',
  doctorCorpusUnset: 'not configured (IRIS_ST_DIR unset)',
  doctorCorpusOk: '{path} is readable',
  doctorCorpusUnreadable: '{path} cannot be listed',
  doctorCorpusUnreadableFix: 'fix IRIS_ST_DIR (the SillyTavern profile directory) and restart — preset import reads from it',
  ...
  /**
   * The plugin center (`PluginCenter.tsx`), the settings drawer's catalog of
   * the host's bundled system plugins. The head's two counts are suffix
   * fragments — the component prints the number in front of them, so the
   * English fragments stay lower-case and the Chinese ones carry the measure
   * word (「2 个已安装」).
   */
  pluginCenterLead: 'System plugins provide optional runtime capabilities to cards. Card script permission is managed separately.',
  pluginCenterBundled: 'Bundled catalog',
  pluginCenterInstalled: 'installed',
  pluginCenterActive: 'active',
  pluginCenterLoading: 'Reading the system plugin catalog…',
  pluginCenterUnavailable: 'The plugin catalog is unavailable while Iris is offline.',
  pluginCenterLoadFailed: 'The plugin catalog could not be read. {detail}',
  pluginCenterRetry: 'Retry',
  pluginCenterVersion: 'Version',
  pluginCenterApi: 'Plugin API',
  pluginCenterDependencies: 'Requires',
  pluginCenterNone: 'None',
  pluginCenterRetained: 'Uninstalling keeps card and chat data, stored variables, and plugin settings. Bundled plugins can be reinstalled without a download.',
  pluginCenterPermission: 'Enabling a system plugin does not allow card scripts or grant page access.',
  pluginCenterInstall: 'Reinstall',
  pluginCenterUninstall: 'Uninstall',
  pluginCenterEnable: 'Enable',
  pluginCenterRetryEnable: 'Retry enable',
  pluginCenterDisable: 'Disable',
  pluginCenterReload: 'Reload',
  pluginCenterInstalling: 'Reinstalling…',
  pluginCenterUninstalling: 'Uninstalling…',
  pluginCenterEnabling: 'Enabling…',
  pluginCenterDisabling: 'Disabling…',
  pluginCenterReloading: 'Reloading…',
  pluginCenterRequestSent: 'Request sent. Waiting for Iris to report the result.',
  pluginCenterWaitFor: 'Wait for {name} to finish before changing another plugin.',
  pluginCenterOperationFailed: 'Iris could not change {name}. {detail}',
  ...
  pluginCenterStatusEnabling: 'Enabling…',
  pluginCenterStatusEnabled: 'Enabled',
  pluginCenterStatusDisabling: 'Disabling…',
  pluginCenterStatusError: 'Error',
  /** The browser-asset half of the status surface, beside the host chip. */
  pluginCenterHostRuntime: 'Host runtime',
  pluginCenterBrowserAsset: 'Browser asset',
  pluginCenterAssetPhaseUndeclared: 'Not declared',
  pluginCenterAssetPhaseLoading: 'Loading…',
  pluginCenterAssetPhaseLoaded: 'Loaded',
  pluginCenterAssetPhaseDegraded: 'Degraded',
  pluginCenterAssetPhaseStale: 'Stale revision',
  pluginCenterExpectedRevision: 'Catalog revision',
  pluginCenterManifestRevision: 'Manifest revision',
  ...
  /** The six failure kinds the asset probe classifies, one sentence each. */
  pluginCenterAssetErrorClientMissing: 'client.js does not exist.',
  pluginCenterAssetErrorHttp: 'HTTP fetch of the browser asset failed.',
  pluginCenterAssetErrorParse: 'The browser asset failed JavaScript parsing.',
  pluginCenterAssetErrorManifest: 'The plugin manifest is malformed.',
  pluginCenterAssetErrorRevision: 'The loaded manifest does not match the current revision.',
  pluginCenterAssetErrorConflict: 'Member name conflict between plugins.',
  pluginCenterRetentionAria: 'Data retention',

  /**
   * The package install path (`docs/SYSTEM-PLUGIN-INSTALL.md` §8, PR-3).
   *
   ...
   * guarantee is that there is no new interface"), the permissions note (§12
   * ruling 4 — a declaration the host shows and spell-checks, never a boundary
   * it enforces), and the `dev` marker (§12 ruling 1 — the disclosure that pays
   * for the missing hash check).
   */
  pluginCenterInstallSection: 'Install a package',
  pluginCenterInstallLead: 'A system plugin runs inside Iris with the same privileges Iris has. Install one only from a source you trust.',
  pluginCenterInstallOpen: 'Install a package…',
  pluginCenterInstallDismiss: 'Close the install form',
  pluginCenterInstallModeLabel: 'Source',
  pluginCenterInstallModeGit: 'Git remote',
  pluginCenterInstallModeDev: 'Local directory (dev)',
  pluginCenterInstallRemote: 'Remote URL',
  pluginCenterInstallRemoteHint: 'An https URL. It is recorded in the catalog and shown again before anything is installed.',
  pluginCenterInstallCommit: 'Commit',
  pluginCenterInstallCommitHint: 'A full 40-character commit. A branch or a tag moves, which would make the recorded provenance a lie.',
  pluginCenterInstallPath: 'Directory path',
  pluginCenterInstallPathHint: 'A dev package is loaded in place from your own directory. Its bytes are never re-checked against a recorded hash, so the row stays marked dev for as long as it exists.',
  pluginCenterInstallSubmit: 'Stage and review',
  pluginCenterInstallStaging: 'Staging the package…',
  pluginCenterInstallNeedRemote: 'Enter the remote URL of the package repository.',
  pluginCenterInstallLongRemote: 'That remote URL is longer than 2000 characters, which Iris will not send.',
  pluginCenterInstallBadCommit: 'A commit is exactly 40 lowercase hexadecimal characters. Iris will not send a branch, a tag or a short sha.',
  pluginCenterInstallNeedPath: 'Enter the path of the package directory.',
  pluginCenterInstallLongPath: 'That directory path is longer than 1000 characters, which Iris will not send.',
  pluginCenterInstallRefused: 'Iris did not stage the package. {detail}',

  pluginCenterConsentTitle: 'Review {name} before installing it',
  pluginCenterConsentAria: 'What this package is, and what installing it means',
  pluginCenterConsentPrivilege: 'This is host code. Once enabled it runs with everything Iris can reach: your conversations, your files, your API keys. Iris does not sandbox a system plugin, and nothing on this page is a limit on what the package can do.',
  pluginCenterConsentDevNote: 'A dev package is loaded in place from the directory below. Its bytes are never re-verified against a recorded hash, which is why this package stays marked dev wherever it appears.',
  pluginCenterConsentAfter: 'Installing is not enabling. The package is added disabled; its host code is first imported when you enable it.',
  pluginCenterConsentId: 'Package id',
  pluginCenterConsentName: 'Name',
  pluginCenterConsentDescription: 'Description',
  pluginCenterConsentVersion: 'Version',
  pluginCenterConsentApi: 'Plugin API',
  pluginCenterConsentSupported: 'Supported here',
  pluginCenterConsentCompatible: 'Compatible',
  pluginCenterConsentApiOk: 'Yes — this build implements it.',
  pluginCenterConsentApiBad: 'No — this build does not implement it.',
  pluginCenterConsentIncompatible: 'This build cannot run this package, so it cannot be installed. Ask its author for a release built against a plugin API this build supports.',
  pluginCenterConsentSource: 'Source',
  pluginCenterConsentRemote: 'Remote',
  pluginCenterConsentCommit: 'Commit',
  pluginCenterConsentPath: 'Directory',
  pluginCenterConsentTreeHash: 'Tree hash',
  pluginCenterConsentTreeHashNote: 'The hash of exactly the bytes already staged on this machine. Confirming approves these bytes and nothing else.',
  pluginCenterConsentFiles: 'Files',
  pluginCenterConsentSize: 'Size',
  pluginCenterConsentCapabilities: 'Provides',
  pluginCenterConsentCapabilitiesNote: 'What the author says the package provides. Iris has no registry to check this against.',
  pluginCenterConsentPermissions: 'Declares it uses',
  pluginCenterConsentPermissionsNote: 'A declaration the author writes, which Iris only spell-checks and shows. It is not a boundary Iris enforces: a system plugin is host code, and it can do everything on this list and everything not on it.',
  pluginCenterConsentDependencies: 'Requires',
  ...
  pluginCenterConsentInstalling: 'Installing…',
  pluginCenterConsentRefused: 'Iris refused the install. {detail}',

  pluginCenterSourceAria: 'Where this plugin came from',
  pluginCenterSourceBuiltin: 'bundled',
  pluginCenterSourceGit: 'git',
  pluginCenterSourceDev: 'dev',
  pluginCenterSourceDevTitle: 'Loaded in place from a directory on this machine. Its bytes are never re-verified.',
  pluginCenterProvenance: 'Recorded at install',
  pluginCenterProvenanceExpand: 'Full remote, commit and tree hash',
  ...
  pluginCenterFailureStep: 'Git step: {step}',
  pluginCenterFailureInstallFailed: 'The package never finished installing.',
  pluginCenterFailureFixInstallFailed: 'Nothing from it was imported. Uninstall this row, then install the package again.',
  pluginCenterFailureManifestInvalid: 'The package manifest is not something Iris can read.',
  pluginCenterFailureFixManifestInvalid: 'Only the author can fix this. Uninstall this row, then install a release whose manifest names the field above correctly.',
  pluginCenterFailureIncompatible: 'The package was built against a plugin API this build of Iris does not implement.',
  pluginCenterFailureFixIncompatible: 'Iris will not enable it. Ask the author for a release built against a plugin API this build supports.',
  pluginCenterFailureTampered: 'The files on disk are not the files Iris recorded when this package was installed.',
  pluginCenterFailureFixTampered: 'Iris will not import them. Reinstall from the recorded remote and commit, which fetches the approved bytes again and asks you to review them.',
  pluginCenterFailureLoadFailed: 'The package threw while Iris was loading it, before any of it started.',
...
export const zh: Record<StringKey, string> = {
  ...
  turnNavigationLabel: '对话导航',
  turnNavigationJump: '跳转到第 {n} 段对话',
  turnNavigationEntry: '第 {n} 段对话',
  ...
  pluginFixFacadeMismatch: '它是为更新版本的 Iris 写的。升级 Iris，或者再说一句话让它按这一版重写。',
  pluginVersionLabel: 'v{version}',
  ...
  treeActionsFor: '「{title}」的操作',
  treeRename: '重命名',
  treeRenameAria: '「{title}」的新名字',
  treeDelete: '删除分支',
  treeDeleteTitle: '删除「{title}」？',
  treeDeleteFloors: '「{title}」有 {floors} 层。删掉的是它的文件；备份会保留，可以从快照恢复。',
  treeDeleteReattach: '它的 {n} 个子分支会留下，接到「{parent}」上，分叉点按它们真正分开的那一层重新算。',
  treeDeletePromote: '这是这棵树的根。最早分出去的「{child}」会成为新的根，其余子分支接到它上面。',
  treeDeleteCascade: '同时删除它下面的 {n} 个子分支（一共 {total} 段对话）',
  treeDeleteViewing: '你正在读这段对话，删除后会转到「{next}」。',
  treeDeleteConfirm: '删除',
  treeDeleteConfirmMany: '删除 {n} 段对话',
  treeActivity: '{title}\n线宽 {width}px，活跃度 {pct}%：分叉后自己写了 {own} 层，最近一次活动比这家族最新的早 {age}。\n线宽 1.5–4px：七成看自己的楼层数（对数，64 层封顶），三成看最近活动（每 7 天减半）。',
  treeAgeNone: '不到 1 小时',
  treeAgeHours: '{n} 小时',
  treeAgeDays: '{n} 天',
  ...
  compareMode: '比较变量',
  compareModeTitle: '在树图上点两层，比较它们的变量。任何时候按住 Shift 点一层也行；Esc 退出。',
  comparePickA: '在下面的树图上点一层，选作 A。',
  comparePickB: '再点一层，选作 B。',
  compareAria: '变量比较',
  compareSwap: '交换 A 和 B',
  compareClose: '关闭比较',
  compareFloorAria: '{side} 侧的楼层',
  compareReading: '第 {n} 个读法',
  compareLoading: '正在比较…',
  compareIdentical: '这两层的变量完全相同。',
  compareNoChanges: '这个筛选下没有差异。',
  compareMissing: '{side} 在这里没有变量：{why}。',
  compareMissingFloor: '没有这一层',
  compareMissingSwipe: '没有这个读法',
  compareMissingTable: '这一层从没写过变量',
  compareFromTurn: '{side} 是用户楼层；显示的是它所在回合的变量表，也就是卡片读这一层时拿到的那份。',
  comparePruned: '{side} 的变量表被清理过；比较的是留下来的部分。',
  compareByIndex: '列表按位置逐项比较。',
  compareRemovedHead: '只在 A 中',
  compareMarkTitle: '比较的 {side} 侧',
  compareSummaryAria: '只在 B 中 {added} 项，改变 {changed} 项，只在 A 中 {removed} 项',
  forkCompare: '比较变量：',
  forkDeleteGroup: '删除',
  ...
  networkAccess: '网络访问权',
  networkGrantedNote: '这张卡的界面可以从任意 https 主机加载图片、样式和字体，也可以发起请求——整张界面住在作者服务器上的卡需要的正是这个。帧里能看到的一切都能顺着这条通道离开，包括你眼前的对话。http 一律拒绝；远程代码无论是否授权都只在白名单内。在你关闭之前一直如此。',
  ...
  networkGrantDialogCancel: '保持关闭',
  networkGrantDialogConfirm: '授予网络访问权',
  runsWithCard: '随这张卡运行',
  ...
  commandDoctorSummary: '检查宿主、这个页面和这个对话，并说该修什么',
  commandDoctorFailed: '/doctor 没能跑完 —— {reason}',
  doctorHeading: '体检：{ok} 项正常，{warn} 项要留意，{fail} 项要修',
  doctorRowWithRemedy: '{row} —— {remedy}',
  doctorUnreadable: '读不到（{reason}）',
  doctorLabelHost: '宿主',
  doctorLabelBundle: '页面构建',
  doctorLabelProvider: '提供方',
  doctorLabelProviderTest: '提供方测试',
  doctorLabelAuthoring: '写插件',
  doctorLabelConsent: '卡片脚本',
  doctorLabelDataDir: '数据目录',
  doctorLabelFaults: '宿主故障',
  doctorLabelSandbox: '卡片沙箱',
  doctorLabelPolicy: '页面策略',
  doctorLabelPlugins: '系统插件',
  doctorLabelStorage: '卡片存储',
  doctorLabelCorpus: 'SillyTavern 目录',
  doctorHostRead: 'Iris {version}，Node {node}，进程 {pid}，端口 {port}',
  doctorHostUnreadable: '宿主没有回答体检的提问（{reason}），所以构建、数据目录、存储和 SillyTavern 这几行跳过',
  doctorHostUnreadableFix: '比这个页面旧的宿主不认识这个提问——用当前构建重启宿主，再刷新页面',
  doctorBundleNotBuilt: '这个页面是从 {page} 启动的，不是构建产物，所以没有比对',
  doctorBundleHostNone: '这个页面跑的是 {page}，而宿主没有在提供构建好的界面',
  doctorBundleHostNoneFix: '在 apps/iris-web 里运行 npm run build，然后重启宿主',
  doctorBundleHostUnreadable: '这个页面跑的是 {page}，而宿主构建出的首页里读不到入口',
  doctorBundleHostUnreadableFix: '重新构建界面（apps/iris-web 里 npm run build）',
  doctorBundleStale: '这个页面跑的是 {page}，但宿主现在提供的是 {host}',
  doctorBundleStaleFix: '刷新页面——它还在跑宿主换构建之前的那一版，新功能都不在',
  doctorBundleSame: '这个页面跑的是 {entry}，就是宿主现在提供的那一版',
  doctorProviderMissing: '没有在用的提供方',
  doctorProviderMissingFix: '到 设置 → 连接 添加一个并点「使用」；在那之前什么都生成不了',
  doctorProviderInUse: '{name}，模型 {model}',
  doctorProviderAnswered: '{ms} 毫秒内有回应',
  doctorProviderRefused: '没有回应：{reason}',
  doctorProviderRefusedFix: '到 设置 → 连接 打开这个提供方，核对地址和密钥',
  doctorAuthoringUnset: '还没有选写插件用的模型，所以「创造」是灰的',
  doctorAuthoringOrphan: '写插件用的模型 {model} 指向一个已经不存在的提供方',
  doctorAuthoringUnsetFix: '到 设置 → 连接，在「写插件用」那一行选一个',
  doctorAuthoringSet: '{name} 上的 {model}',
  doctorConsentNone: '这张卡没有脚本',
  doctorConsentUnasked: '这张卡的 {n} 个脚本在等你回答，所以还没有运行',
  doctorConsentAskFix: '回答对话上方的那个提问',
  doctorConsentDeclined: '你拒绝了这张卡的 {n} 个脚本，所以它们不运行',
  doctorConsentDeclinedFix: '如果这张卡需要它们，到 设置 → 脚本 里允许',
  doctorConsentAllowed: '已允许 {n} 个脚本；没有授予远程网络',
  doctorConsentAllowedNet: '已允许 {n} 个脚本；已授予远程网络',
  doctorDataDirOk: '{path}，可写，锁由这个宿主持有（进程 {pid}）',
  doctorDataDirNotWritable: '宿主报告 {path} 不可写',
  doctorDataDirNotWritableFix: '修好目录权限，或者把 IRIS_DATA_DIR 指向一份可写的副本再重启',
  doctorLockMissing: '{path} 里没有 host.lock',
  doctorLockMissingFix: '重启宿主让它重新取锁——在那之前，另一个宿主也能打开同一个目录',
  doctorLockOther: '{path} 里的 host.lock 记的是进程 {lock}，不是这个宿主的 {pid}',
  doctorLockOtherFix: '另一个宿主打开了这个目录；停掉其中一个，两个宿主共用一个目录会互相覆盖保存',
  doctorFaultsNone: '宿主这次打开以来没有留存故障',
  doctorFaultsHeld: '留存了 {n} 条故障；最新一条（{kind}）：{message}',
  doctorFaultsFix: '设置 → 诊断 → 宿主报告 逐条列出了原因',
  doctorSandboxOk: '清单和 {preset} 都能取到',
  doctorSandboxBroken: '沙箱构建取不到：{reason}',
  doctorSandboxFix: '重新构建界面（apps/iris-web 里 npm run build）——没有它，卡片脚本和界面都起不来',
  doctorPolicyOk: '这个页面带着外壳的内容安全策略',
  doctorPolicyMissing: '这个页面没有外壳的内容安全策略',
  doctorPolicyMissingFix: '从 Iris 宿主自己的地址打开界面，而不是从开发服务器',
  doctorPluginsOk: '已安装 {n} 个，没有失败的',
  doctorPluginsFailed: '失败：{plugins}',
  doctorPluginsFix: '设置 → 系统插件 里能看到失败原因和重试',
  doctorStorageRead: '已用 {used}，上限 {limit}',
  doctorStorageFullFix: '快满了，卡的下一次写入会被拒绝；清掉一张旧卡的存储',
  doctorCorpusUnset: '没有配置（IRIS_ST_DIR 未设置）',
  doctorCorpusOk: '{path} 可以读取',
  doctorCorpusUnreadable: '{path} 列不出内容',
  doctorCorpusUnreadableFix: '改正 IRIS_ST_DIR（SillyTavern 的 profile 目录）再重启——导入预设要从它读',
  ...
  /** 插件中心（`PluginCenter.tsx`）：设置抽屉里宿主内置插件的目录页。 */
  pluginCenterLead: '系统插件为卡片提供可选的运行能力。卡片脚本权限仍在脚本页面中单独管理。',
  pluginCenterBundled: '内置插件目录',
  pluginCenterInstalled: '个已安装',
  pluginCenterActive: '个运行中',
  pluginCenterLoading: '正在读取系统插件目录…',
  pluginCenterUnavailable: 'Iris 离线时无法读取插件目录。',
  pluginCenterLoadFailed: '无法读取插件目录。{detail}',
  pluginCenterRetry: '重试',
  pluginCenterVersion: '版本',
  pluginCenterApi: '插件 API',
  pluginCenterDependencies: '依赖',
  pluginCenterNone: '无',
  pluginCenterRetained: '卸载会保留卡片与聊天数据、已存变量和插件设置。内置插件可直接重新安装，无需下载。',
  pluginCenterPermission: '启用系统插件不会允许卡片脚本运行，也不会授予页面访问权。',
  pluginCenterInstall: '重新安装',
  pluginCenterUninstall: '卸载',
  pluginCenterEnable: '启用',
  pluginCenterRetryEnable: '重试启用',
  pluginCenterDisable: '停用',
  pluginCenterReload: '重新加载',
  pluginCenterInstalling: '正在重新安装…',
  pluginCenterUninstalling: '正在卸载…',
  pluginCenterEnabling: '正在启用…',
  pluginCenterDisabling: '正在停用…',
  pluginCenterReloading: '正在重新加载…',
  pluginCenterRequestSent: '请求已发送，正在等待 Iris 返回实际结果。',
  pluginCenterWaitFor: '请等待 {name} 完成，再更改其他插件。',
  pluginCenterOperationFailed: 'Iris 无法更改 {name}。{detail}',
  pluginCenterReportedError: '宿主错误：',
  pluginCenterFixError: '请先处理上述原因，再重试启用或卸载此插件。',
  pluginCenterBlocked: '请先停用 {names}。',
  ...
  pluginCenterStatusEnabling: '正在启用…',
  pluginCenterStatusEnabled: '已启用',
  pluginCenterStatusDisabling: '正在停用…',
  pluginCenterStatusError: '错误',
  /** 状态面上的浏览器资产一半，在宿主启用章旁边。 */
  pluginCenterHostRuntime: '宿主运行时',
  pluginCenterBrowserAsset: '浏览器资产',
  pluginCenterAssetPhaseUndeclared: '未声明',
  pluginCenterAssetPhaseLoading: '加载中…',
  pluginCenterAssetPhaseLoaded: '已加载',
  pluginCenterAssetPhaseDegraded: '降级',
  pluginCenterAssetPhaseStale: '旧 revision',
  pluginCenterExpectedRevision: '目录 revision',
  pluginCenterManifestRevision: '清单 revision',
  ...
  /** 资产探测归类出的六种故障，各一句。 */
  pluginCenterAssetErrorClientMissing: 'client.js 不存在。',
  pluginCenterAssetErrorHttp: '浏览器资产 HTTP 获取失败。',
  pluginCenterAssetErrorParse: '浏览器资产 JavaScript 解析失败。',
  pluginCenterAssetErrorManifest: '插件清单格式错误。',
  pluginCenterAssetErrorRevision: '已加载清单与当前 revision 不匹配。',
  pluginCenterAssetErrorConflict: '插件之间存在成员名冲突。',
  pluginCenterRetentionAria: '数据保留说明',

  /** 插件包安装路径（`docs/SYSTEM-PLUGIN-INSTALL.md` §8，PR-3）：表单、同意页、行上的来源/来源记录/失败状态。 */
  pluginCenterInstallSection: '安装插件包',
  pluginCenterInstallLead: '系统插件在 Iris 内运行，权限与 Iris 本身相同。只从你信任的来源安装。',
  pluginCenterInstallOpen: '安装插件包…',
  pluginCenterInstallDismiss: '收起安装表单',
  pluginCenterInstallModeLabel: '来源',
  pluginCenterInstallModeGit: 'git 远端',
  pluginCenterInstallModeDev: '本地目录（dev）',
  pluginCenterInstallRemote: '远端 URL',
  pluginCenterInstallRemoteHint: '一个 https URL。它会被记进目录，并在任何东西被安装之前再展示一次。',
  pluginCenterInstallCommit: 'commit 值',
  pluginCenterInstallCommitHint: '完整的 40 位 commit。分支和标签会移动，那会让记录下来的来源说谎。',
  pluginCenterInstallPath: '目录路径',
  pluginCenterInstallPathHint: 'dev 包直接从你自己的目录就地加载，它的字节永远不会与记录下来的哈希复核，所以这一行会一直标着 dev。',
  pluginCenterInstallSubmit: '暂存并查看',
  pluginCenterInstallStaging: '正在暂存这个包…',
  pluginCenterInstallNeedRemote: '请填写这个包所在仓库的远端 URL。',
  pluginCenterInstallLongRemote: '这个远端 URL 超过 2000 个字符，Iris 不会发送它。',
  pluginCenterInstallBadCommit: 'commit 必须恰好是 40 位小写十六进制字符。分支、标签和短 sha，Iris 都不会发送。',
  pluginCenterInstallNeedPath: '请填写这个包所在目录的路径。',
  pluginCenterInstallLongPath: '这个目录路径超过 1000 个字符，Iris 不会发送它。',
  pluginCenterInstallRefused: 'Iris 没有暂存这个包。{detail}',

  pluginCenterConsentTitle: '安装 {name} 之前，请先看清楚',
  pluginCenterConsentAria: '这个包是什么，以及安装它意味着什么',
  pluginCenterConsentPrivilege: '这是宿主代码。一旦启用，它能触及 Iris 能触及的一切：你的对话、你的文件、你的 API 密钥。Iris 不给系统插件沙箱，本页上的任何一项都不是对它能做什么的限制。',
  pluginCenterConsentDevNote: 'dev 包直接从下面这个目录就地加载，它的字节永远不会与记录下来的哈希复核——这就是它出现在哪里都标着 dev 的原因。',
  pluginCenterConsentAfter: '安装不等于启用。这个包会以停用状态加入目录，它的宿主代码要等你点启用时才第一次被载入。',
  pluginCenterConsentId: '包 id',
  pluginCenterConsentName: '名称',
  pluginCenterConsentDescription: '描述',
  pluginCenterConsentVersion: '版本',
  pluginCenterConsentApi: '插件 API',
  pluginCenterConsentSupported: '本机支持',
  pluginCenterConsentCompatible: '是否兼容',
  pluginCenterConsentApiOk: '兼容——本次构建实现了它。',
  pluginCenterConsentApiBad: '不兼容——本次构建没有实现它。',
  pluginCenterConsentIncompatible: '本次构建跑不了这个包，所以它不能被安装。请向作者要一个针对本构建支持的插件 API 发布的版本。',
  pluginCenterConsentSource: '来源',
  pluginCenterConsentRemote: '远端',
  pluginCenterConsentCommit: 'commit 值',
  pluginCenterConsentPath: '目录',
  pluginCenterConsentTreeHash: '树哈希',
  pluginCenterConsentTreeHashNote: '这是已经暂存在本机上的那一份字节的哈希。确认即是同意这一份字节，仅此而已。',
  pluginCenterConsentFiles: '文件数',
  pluginCenterConsentSize: '体量',
  pluginCenterConsentCapabilities: '声称提供',
  pluginCenterConsentCapabilitiesNote: '这是作者声称这个包提供什么。Iris 没有能力注册表可以校验它。',
  pluginCenterConsentPermissions: '声称会用到',
  pluginCenterConsentPermissionsNote: '这是作者写下的声明，Iris 只做拼写校验并展示。它不是 Iris 强制的边界：系统插件是宿主代码，这张表上的事它能做，不在这张表上的事它也能做。',
  pluginCenterConsentDependencies: '依赖',
  ...
  pluginCenterConsentInstalling: '正在安装…',
  pluginCenterConsentRefused: 'Iris 拒绝了这次安装。{detail}',

  pluginCenterSourceAria: '这个插件的字节从哪来',
  pluginCenterSourceBuiltin: '内置',
  pluginCenterSourceGit: 'git 远端',
  pluginCenterSourceDev: 'dev 本地',
  pluginCenterSourceDevTitle: '从本机的一个目录就地加载，它的字节永远不会被复核。',
  pluginCenterProvenance: '安装时记录',
  pluginCenterProvenanceExpand: '展开完整的远端、commit 与树哈希',
  ...
  pluginCenterFailureStep: 'git 步骤：{step}',
  pluginCenterFailureInstallFailed: '这个包没有安装完。',
  pluginCenterFailureFixInstallFailed: '它没有任何东西被载入过。请卸载这一行，然后重新安装这个包。',
  pluginCenterFailureManifestInvalid: '这个包的清单不是 Iris 读得懂的形状。',
  pluginCenterFailureFixManifestInvalid: '这只能由作者修。请卸载这一行，然后安装一个清单里上述字段写对了的版本。',
  pluginCenterFailureIncompatible: '这个包是针对本次构建没有实现的插件 API 做的。',
  pluginCenterFailureFixIncompatible: 'Iris 不会启用它。请向作者要一个针对本构建支持的插件 API 发布的版本。',
  pluginCenterFailureTampered: '磁盘上的文件与安装时 Iris 记录下来的不符。',
  pluginCenterFailureFixTampered: 'Iris 不会载入它们。请按记录的远端与 commit 重新安装——那会把当初同意过的字节重新拉一次，并请你再看一遍。',
  pluginCenterFailureLoadFailed: 'Iris 载入这个包时它就抛了错，它还没开始运行。',
