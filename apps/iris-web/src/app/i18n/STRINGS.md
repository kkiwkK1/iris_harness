...
| `ScriptPanel.tsx` | 面板头、读取中、无脚本、已拒绝摘要、允许脚本、撤回/更早运行注记、页面访问权全部文案、授权风险对话框四条、网络访问权全部文案与其风险对话框五条（2026-09-19 随网络授权契约落地）、拒绝行上的授权入口与「已开启」句（2026-09-19，见 web 台账 §116）、随卡运行/你已关闭、卡内关闭、运行它们/不运行 | `sectionCardScripts readingCard cardNoScripts declined* allowScripts runThem dontRunThem withdrawn fromEarlierRun pageAccess* grantDialog* networkAccess* networkGrantedNote networkOffNote networkGrantDialog* turnOffNetworkAccess giveNetworkAccess grantNetworkFromReport networkGrantAlreadyOn runsWithCard youTurnedThisOff cardOffNote` |
...
同日 M1 第一至四步（装配面板的解释、按消息视图、宏与 regex 阶段、预算账）继续加，
读到 **1220 / 1220**。
...
## 侧栏重做（2026-09-10，`dev/sidebar-redesign`）

| 来源 | 内容 | 键 |
| --- | --- | --- |
| `format.ts` `chatMeta` | 对话行的整条戳记：`since` ＋短计数，「2 天前 · 3 条」。行从两行压成一行，戳记成了一列，所以短计数是新的 | `messageCountShort` |
| `Sidebar.tsx` 头部 | 折叠钮与轨道里的展开钮各自的 aria。两枚控件、一个状态：各只在自己那句话成立的那一态里可见（`Sidebar.tsx` 说了为什么不是一枚会位移的钮） | `collapseSidebar expandSidebar` |
| `Sidebar.tsx` 对话排序 | 两枚胶囊的分组 aria 与两个词。**拖过一次之后才出现**——在此之前宿主没有排列，两个答案是同一份列表，一排永远不改变任何东西的胶囊是控件里最糟的一种 | `chatOrderAria chatOrderManual chatOrderRecent` |
| 轨道三枚图标 | 没有新键：图标的 aria 与 title 复用 `tabReading` / `tabCharacters` / `chatSearchAria` / `importCard`，因为轨道图标去的正是展开态里那四个地方。为一枚图标造一句只在 44px 下用的文案，就是同一个目的地在两本词典里各有两个名字 | （复用） |

**`messageCount` 留着，没有被短式替换。** 长式（`{count} 条消息`）现在只有角色页那一列
在用，而那一列宽到说得下这个名词；侧栏那一行说不下。两个键各有一个调用者，删掉长式会让
角色页那一列变成一个缺名词的数字。

**「自定义」不是 `Custom`。** 英文取 `Arranged`：它说的是读者做过的那个动作，而 `Custom`
是设置面板给一个自己讲不清的值起的名字。中文取「自定义」，因为「已排列」在中文里读起来像
一句状态而不是一个排序选项。
...
## 装配面板的宏与 regex 阶段（2026-09-16，M1 第三步）

| 来源 | 内容 | 键 |
| --- | --- | --- |
| `PromptPanel.tsx` 的 `Explanation` | 宏阶段一行（`{kinds}` 种宏头、`{top}` 最多的一种及 `{count}` 次、前后字符数） | `promptMacros` |
| 同上 | regex 阶段：命中的规则名列表，或「跑过但没改」 | `promptRegex promptRegexNone` |

**宏那一行的 `{top}` 取次数最多的那个头，不取第一个。** 变量驱动型预设里 `setvar` 跑
61 次而其余各一次，「哪个头、多少次」才是信息量所在；顺序按首次出现，并列时稳定，所以
同一状态渲染两次结果一样。

**`promptRegexNone` 单独一句，不是空串。** 「regex 链跑了、没有规则命中」和「宿主根本没
记录」是两件事；空串会把前者渲染成后者。`regexNote` 对「未记录」返回 `null`（面板什么都
不说），对「记录且为空」返回 `[]`（面板说「跑过，没有改动」）。

**`promptMacros` 只在真采到宏头时出现。** `macroNote` 对「未记录」和「记录了但没解析出
任何头」都返回 `null`：一次没有可解析宏的 trace 与根本没 trace，对读者是同一件事——这一行
没有东西可说。

## 装配面板的预算账（2026-09-16，M1 第四步）

| 来源 | 内容 | 键 |
| --- | --- | --- |
| `PromptPanel.tsx` 的 controls 行 | 更完整的裁剪句：几条楼层、共多少 token | `promptOverflow` |

**`promptOverflow` 与 `droppedToFit` 并存，不是替换。** `droppedToFit` 只说条数，是留给
老宿主记录的——它没有 `overflow` 对象，只有 `droppedHistory` 计数。`overflowNote` 优先读
对象（条数＋重量），没有对象才回退到计数句。**绝不把计数当成重量**印出来：那会替一个没人
量过的数字背书。

**`{floors}` 而不是 `{n}`，与 `droppedToFit` 的 `{n}` 分开。** 两个键在同一个面板相邻出
现，共用一个槽位名会让「哪个键填哪个数」变得靠约定而不是靠名字；分开以后，`i18n.test.ts`
的槽位一致性检查也能分别钉住两句话。

## 引号上色那一格（2026-09-20，web 账本 §122）

| 来源 | 内容 | 键 |
| --- | --- | --- |
| `ReadingPanel.tsx` 的 `ChoiceField` | 「哪些引号算对白」这一格的标题 | `quoteScope` |
| 同上，两个选项 | 默认那一档，和回到上游六种的那一档 | `quoteScopeDialogue` / `quoteScopeUpstream` |

**`quoteScopeUpstream` 两列都带 `SillyTavern` 这个词，而它仍然是要翻译的一行**：
英文是 `Same as SillyTavern (six kinds)`，中文是「与 SillyTavern 一致（六种）」——
产品名不翻，围着它的那句话翻，括号也跟着换成全角。所以它**不在** `i18n.test.ts`
的中立白名单里：白名单是留给数字格式与单位的，不是留给「里面有个英文词」的句子。
