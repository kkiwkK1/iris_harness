...
## 32. The state margin's "this round" is measured against the last snapshot this session witnessed, not against "the previous floor"
...
## 33. A branch renders open down to depth 1, folded below it — and the `toggle` event no longer gets to vote on what the reader did
...
## 34. Long values clamp to two lines with the full text on hover, instead of wrapping without bound
...
## 35. The notice log now merges an identical notice into a counted row, inside a short window
...
## 36. One scrollbar, defined once in the token layer, imposed on every surface with `*`
