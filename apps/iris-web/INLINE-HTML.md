...
这件事是从「爱衣的状态栏显示成文本」发起的，当时的诊断是「片段没被消毒渲染」。
**那个诊断被语料推翻了**：

- 爱衣**只有 1 楼**，渲染后 `is_frontend = YES`，含 `html`/`head`/`body`/`script`/`style`——
  它是**完整文档**，该走 frame 路径。markdown 关掉时它渲染出 3837 字符的**纯叙事、零标签**。
  所以爱衣是**完整文档被 frame 路径漏接**（7b 在查，疑似 claim 只认围栏块），不是片段问题。
- **但片段带真实存在，而且是另一批卡**：936 楼，887 楼在命定之诗（用户的 677 楼主力长局）。

**所以这份文档的适用面是那 936 楼，不是爱衣。**修好内联消毒不会让爱衣显示出来；
修好 frame 路径的漏接才会。两件事，两个修法，别合并。

写在最前面，是因为**一份在错前提下委托的文档，如果不说，读者会默认它治的是委托时那个病。**
...
语料侧的相关事实（我自己量的，口径是**渲染前**的磁盘文本）：
`<style type="text/tailwindcss">` 在 3 个世界书文件里出现 **4 次**
（`worlds/命定之诗与黄昏之歌v3.0.4.json`、`worlds/板板鸭 v2.0.json`、`worlds/萧谴写卡助手版_V4.5.1.json`）。
带 tailwind 的几乎必然在完整文档里、走 frame 路径，**所以这 4 处大概率打不到这个洞**。
但**「带属性的 `<style>` 这种写法在这个用户的语料里是存在的」**已经成立。
（`chats/` 下渲染前 `<style` 是零——那 24 楼的 `<style>` 是渲染期正则展开出来的，
所以它们的**开标签确切形状我看不到**，需要 44 那边补一次「渲染后 `<style` 的开标签形态」普查。列入 §七。）
...
1. **渲染后 `<style>` 的开标签确切形状**（有没有属性）——44 那边能查，我这边看不到，
   因为 `chats/` 渲染前 `<style` 是零。它决定 §2.3 那个洞在我们语料里是不是真被踩到。
   *不影响结论*：无论踩没踩到，我们的做法都是「在 DOM 上处理」。
2. **爱衣那条完整文档为什么被 frame 路径漏接**——7b 在查，不在这份文档的范围里（§〇）。
...
**44 量的、我转述**：§五整节的语料数字，以及 §〇 爱衣的双渲染对照。
