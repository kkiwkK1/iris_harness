...
Iris 是 **SillyTavern 的升级版,不是替代品**。它直接吃 SillyTavern 的生态资产——角色卡、世界书、预设、聊天记录、酒馆助手脚本、MVU 变量——并把它们放进一个阅读式的对话宿主里。**兼容是地板,不是天花板**:能在 SillyTavern 里跑的东西,在这里也该跑;跑不了或跑得不一样的地方,逐条记在偏离账本里,写清代价与「什么会推翻这条」;在此之上的改进,作为有文档的功能单独记账。
...
- **`IRIS_BASE_URL`** 是 OpenAI 兼容端点的根,`/chat/completions` 由程序追加;**`IRIS_MODEL`** 是新聊天默认用的模型 id。
...
打开界面右上角的**设置**,第一张卡就是**连接**。

选一个 **provider 预置**,`baseURL` 会自动填好:
...
测试成功后模型列表会喂给一个可搜索的下拉框,**你仍然可以自己输入**列表里没有的模型名。**切换端点或密钥不需要重启宿主**:激活一个 profile 会在运行时重新装配适配器,重启后仍然记得上次激活的那个。
