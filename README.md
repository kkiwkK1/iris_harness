...
SillyTavern 的**升级版**（~~替代品~~，2026-09-02 重定位：兼容是地板，不是天花板），地基换成 DeepSeek Harness 所用的 **Cordis** 插件框架——每个组件（模型适配器、提示词装配、世界书、持久化、UI 区块）都是可挂载/可卸载/可热重载的插件，卸载时副作用完整回滚。

目标分两层。**地板**：完整承接 SillyTavern 的领域能力与生态资产（角色卡、世界书、预设、聊天记录、酒馆助手脚本、MVU 变量），兼容面上的偏离逐条记档。**其上**：在 ST 做不到或不肯做的地方做得更好——卡片代码跑在真隔离沙箱里而不是页面全权；每一次失败有名字、有落点、可撤回（具名拒绝、常驻报告、遗言通道、判决可撤销）；远程资产经宿主代取缓存而不看 CDN 脸色；存储形状经真实语料校准而不是照文档臆写。升级的纪律与兼容相同：每处"比上游好"都要写明好在哪、代价是什么、上游为什么没这么做。
...
**1094 个测试（1092 通过 / 2 联网跳过）**，类型检查干净。架构不变量（依赖分层、浏览器
import 白名单等）由测试强制（[ARCHITECTURE.md](ARCHITECTURE.md)）。

端到端在真实数据上验证过：真页面 → 真传输 → 真宿主 → 真卡。两条主线各有一张真卡作证:
**变量折叠**——真模型（deepseek-v4-pro）对真卡回复 `<JSONPatch>`，宿主折叠、状态面板
显示（与真实 SillyTavern 语料 1/1 合规对照后修掉两个宿主缺口才通）;**卡片脚本**——
语料里最难的多脚本卡上,MVU 生产 bundle 经宿主代理取回、注册、赢得选举、发布接口,
**四个脚本全部 loaded and listening,发布链端到端、消费者全部醒来**（沙箱策略与实测见
