...
供应商列表下面还有一行「写插件用」。它选的是：当你把输入框切到「创造」、说一句话让这张卡长出一个功能时，**由哪个供应商的哪个模型把这个插件写出来**（见[沙箱插件](SANDBOX-PLUGINS.md)）。

它**刻意不是这个对话正在用的模型**：写代码和演角色很少是同一个模型，而且能写插件的那个常常还在内测、不出现在模型列表里——所以这一行的模型控件和编辑连接时一样，永远可以手打一个名字。

**不设置就用不了。** 没选的时候「创造」是灰的，并且会说明原因；它不会偷偷回落到你正在用的供应商——那会让你用一个自己没选的模型花一笔没预期的钱，而失败的样子看起来像功能坏了，而不像设置没做。写插件那次请求的花费记在这段对话的账上，与卡片脚本自己发起的生成同一个位置：在**用量**页上，它是总量下面单独的一句「其中写插件请求 N 次」，与「卡脚本请求」「压缩摘要」并列，每个对话的小计上也有同样的一格。单列一句是因为它花的是**这一行选的那个供应商**的钱，不是这个对话正在用的那个——嫌这个数大，要改的是这一行的模型或者少说几句「创造」，而不是去动一张卡。没花过就整句不出现，不会印一个 0。
