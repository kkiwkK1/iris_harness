...
**EJS 提示词模板**由插件中心里的内置插件「Iris EJS 模板引擎」（`iris-templates`）提供，默认关闭。使用依赖 ST-Prompt-Template 的角色卡时，在插件中心启用它；启用前会弹出风险确认，因为模板会在这台电脑上执行卡片作者的 JavaScript（运行在受限子进程中）。开关从下一次组装提示词起生效，无需重启。它是 Iris 自带的兼容引擎，**不是** ST-Prompt-Template 扩展本身：若同时启用了已安装的 ST 扩展，提示词中的模板由该扩展展开，Iris 的引擎不再处理提示词（避免重复求值），但仍应答卡片的 `evalTemplate` 调用。以前的环境变量 `IRIS_TEMPLATES` 已退役，见下表。
...
| `IRIS_TEMPLATES` | 已退役 | 不再是开关。仅用于迁移：设为 `1` 且档案的插件目录里还没有「Iris EJS 模板引擎」这一行时，首次启动会把这一行设为启用；之后以插件中心为准。只要仍设置着，每次启动都会在日志中提示它已退役，可以删除 |
...
遇到问题时，先在任意对话的输入框里输入 `/doctor` 并回车。它只读取、不写入，也不花模型 token（「提供方测试」那一行和连接面板的测试按钮一样，只请求模型列表），结果以一条通知显示，点 ✕ 才会关闭，也会留在 **设置 → 诊断 → 通知** 里。每一行是 `✓` 正常、`⚠` 要留意、`✗` 要修，不是 ✓ 的行后面跟着一句该怎么做：

| 行 | 读的是什么 | 常见的 ⚠ / ✗ |
| --- | --- | --- |
| 宿主 | Iris 版本、Node 版本、进程号、端口 | ✗ 宿主不认识体检的提问：宿主比页面旧，用当前构建重启 |
| 页面构建 | 这个页面启动时的入口文件，和宿主现在提供的那一份比 | ⚠ 两者不同：宿主换了构建而页面没刷新，新功能都不在——刷新页面 |
| 提供方 | 在用的提供方和模型 | ✗ 没有在用的提供方：到 设置 → 连接 添加并「使用」 |
| 提供方测试 | 对在用的提供方做一次连接测试 | ✗ 没有回应：核对地址和密钥 |
| 写插件 | 「写插件用」的模型 | ⚠ 未设置，所以「创造」是灰的 |
| 卡片脚本 | 这张卡的脚本授权与远程网络授权 | ⚠ 被拒绝或还在等你回答 |
| 数据目录 | 数据目录路径、是否可写、`host.lock` 记的进程号 | ✗ 锁记的不是这个宿主：另一个宿主开了同一个目录 |
| 宿主故障 | 宿主这次打开以来留存的故障报告 | ⚠ 有故障：到 设置 → 诊断 → 宿主报告 看原因 |
| 卡片沙箱 | 沙箱清单和它指向的库文件能否取到 | ✗ 取不到：重新构建界面 |
| 页面策略 | 这个页面是否带着外壳的内容安全策略 | ⚠ 没有：多半是从开发服务器打开的 |
| 系统插件 | 已安装的系统插件有没有失败的 | ✗ 列出失败的插件和原因 |
| 卡片存储 | 卡片共享存储已用多少、上限多少 | ⚠ 超过九成：卡的写入快要被拒绝 |
| SillyTavern 目录 | `IRIS_ST_DIR` 设置时能否读取 | ✗ 读不到：改正路径再重启 |

对话没有关联角色卡时不出「卡片脚本」一行；宿主没有回答体检提问时，「页面构建」「数据目录」「卡片存储」「SillyTavern 目录」四行跳过，不会猜。
...
| 卡片模板未生效 | 在插件中心检查「Iris EJS 模板引擎」是否已启用；若同时启用了 ST 扩展，模板由扩展展开，需要打开 Iris 页面 |
