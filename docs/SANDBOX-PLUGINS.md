...
- **「收起卡片界面」只收卡，不收这个容器**（2026-09-26）。容器和卡同在 A 族帧里，所以壳在外面把整个覆盖层藏掉会连插件一起藏掉。有插件时壳不藏覆盖层，改发 `card:collapse` 给帧：帧用 `visibility` 藏掉 `body` 下除本容器以外的一切，并且只按本容器量 clip（`apps/iris-web/src/sandbox/card-collapse.ts`）。插件样式在 `head` 里，照常生效。没有插件时仍是壳在外面藏覆盖层，原样。账本见 web DEVIATIONS §26。
