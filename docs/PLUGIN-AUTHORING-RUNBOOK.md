...
**更新（U1，2026-09-15 第二批已实现）**：已安装的 `git` 行可以原位更新到你新推的 commit。用户在该行上点
「更新到…」、输入一个完整 commit，宿主从**行上记录的同一个 remote** 取那棵树、走与安装完全相同的
staging / 契约 / 哈希 / 清单，然后是**同一张同意页**——多出的一行写明「从哪个 commit 更新到哪个」和两个
tree hash。确认后旧目录树被替换、`enabled` 偏好保留（原来开着的更新后仍然开着）；新代启用失败会自动回到
旧代，行上不会留下失败标记，失败的原因在弹出的错误消息里。`dev` 行没有更新事务（就地加载，改文件即生效），
`builtin` 行也没有（随 Iris 本体发布）。对你（作者）的实际含义：发新版本就是推一个 commit 并把它的完整
哈希告诉你的用户，他们不需要卸载重装。
