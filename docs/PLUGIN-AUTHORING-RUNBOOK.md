...
> **文档身份与唯一所有者**
>
> - 文档编号：`DOC-PLUGIN-AUTHOR-RUNBOOK`；它是插件作者使用已经落地接口的操作说明，**不是基础设施施工任务**。
> - 唯一维护者：集成人。插件开发者可以提出修订，但不得在自己的产品分支顺手改写接口状态。
> - 本文件只描述当前可调用合同；新基础设施按 [ST 扩展兼容设计与施工手册](ST-EXTENSION-DESIGN-AND-RUNBOOK.md) §9 的批次、分支和派工单施工，合入后再由集成人更新本文件。
> - “施工手册”如果指本文件，不能再链接到 `ST-EXTENSION-DESIGN-AND-RUNBOOK.md`；两份文档职责不同，不能据此启动两个内容重叠的实现任务。

2026-09-13，针对 `dev/system-plugins @ 934ae1c` 的实现快照。配套 [基础设施接口清单](INFRASTRUCTURE-INTERFACES.md)。本手册包含今天能执行的制作路径与后续发布要求；尚未实现的接口不可照草案调用。后续基线变化以接口清单的核对提交为准。
