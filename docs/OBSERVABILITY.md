...
**表外新长的一条（2026-09-16，M1 第一至四步）：装配面板的「为什么是 0」「落在哪」「经过什么」「预算裁了什么」。** 这十二条
清单里没有它，因为清单是卡脚本的失败面、而它是**提示词装配**的失败面——同一类
可解释性需求长在了第二个地方，记在这里以免下次重查时说我们只有 12 条。
`prompt.itemize` 的每一行现在带 `explanation`：这段字节是谁写的（预设条目 / 卡字段 /
世界书 / 历史 / 脚本注入 / 宿主）、`tokens === 0` 时**为什么**是 0、它进了第几条消息、
是否在缓存前缀里（第二步）、宏与 regex 两阶段做了什么（第三步），以及本轮预算裁掉了
哪些楼层、多少 token（第四步，`overflow`）。实测的那个对话 38 行里 23 行为 0，背后是三个
不同原因，而面板此前把三个渲染成同一个「空」。面板同时多了一个「按消息」视图，列出真正
发出去的每一条消息及它装了哪些 part，并带上与行视图同一份的差异标记——与第 12 条同一个
道理：「某一步为什么这么判」不说出来，读者只能猜。

**表外又一条（2026-09-23）：输入框的 `/doctor`。** 前面十二条都是「某张卡、某个脚本坏在哪」，
这一条是**这次打开本身**坏在哪：页面还在跑宿主换构建之前的包、没有在用的提供方、「写插件用」
没设所以「创造」是灰的、脚本授权被拒、`host.lock` 记的不是这个宿主、缓冲里留着故障、
沙箱构建取不到、页面没带外壳 CSP、系统插件失败、卡片存储快满、`IRIS_ST_DIR` 读不到。
一次读完，出一条不会自己消失的通知，每行 ✓/⚠/✗ + 读到了什么 + 该怎么修。
