...
**表外新长的一条（2026-09-16，M1 第一、二、三步）：装配面板的「为什么是 0」「落在哪」「经过什么」。** 这十二条
...
是否在缓存前缀里（第二步），以及**宏与 regex 两阶段做了什么**（第三步：展开了 61 个
`setvar`、1250 → 0 字符；被哪条规则改写）。实测的那个对话 38 行里 23 行为 0，背后是三个
不同原因，而面板此前把三个渲染成同一个「空」。面板同时多了一个「按消息」视图，列出真正
发出去的每一条消息及它装了哪些 part——与第 12 条同一个道理：「某一步为什么这么判」
不说出来，读者只能猜。
