...
**表外新长的一条（2026-09-16，M1 第一步）：装配面板的「为什么是 0」。** 这十二条
...
世界书 / 历史 / 脚本注入 / 宿主），以及 `tokens === 0` 时**为什么**是 0。实测的那个
对话 38 行里 23 行为 0，背后是三个不同原因（变量驱动型预设、没人填的 marker 槽位、
作者留白的条目），而面板此前把三个渲染成同一个「空」。与第 12 条同一个道理：
「某一步为什么这么判」不说出来，读者只能猜。
