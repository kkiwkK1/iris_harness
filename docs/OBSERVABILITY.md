...
**表外新长的一条（2026-09-16，M1 第一、二步）：装配面板的「为什么是 0」与「落在哪」。** 这十二条
...
世界书 / 历史 / 脚本注入 / 宿主）、`tokens === 0` 时**为什么**是 0、以及它进了第几条消息、
是否在缓存前缀里（第二步）。实测的那个对话 38 行里 23 行为 0，背后是三个不同原因
（变量驱动型预设、没人填的 marker 槽位、作者留白的条目），而面板此前把三个渲染成同一个
