...
**`prompt.itemize` 的响应带解释**（M1 第一、二、三、四步，2026-09-16）。`PromptItemEntry` / `PromptItemMember` 各多一个**可选**字段 `explanation?: { source, zeroReason?, placement?, stable?, macros?, regex? }`：`source` 说明这段字节是谁写的（`preset` / `card` / `worldbook` / `history` / `script` / `host` 六种 kind 加一个 id），`zeroReason` 只在 `tokens === 0` 时出现，写明为什么是 0。今天宿主实际产出两种原因：`macros-only`（原文非空、宏展开后为空——变量驱动型预设的常规形状）与 `marker-unfilled`（槽位被提供、本轮没有内容填进去）。`blank` / `trimmed` / `dropped-by-budget` 在词表里预留给预算那一轮，宿主暂不产出；预设作者留白的条目**不产生行**（与 `emptyMarkerRows` 的取舍一致）。**第二步**加上落点与稳定性：`placement.messageIndex` 是这条 part 进了第几条消息（0 是 system 提示词），`stable` 说它是否在首个易变 part 之前的前缀里。`PromptItemization.messages?: PromptMessageSlot[]` 是反向索引——每条最终消息带 `partIds`、`tokens`、`stable`，与每个 part 的 `placement` 出自同一次装配，所以两个方向不可能互相矛盾（宿主侧有往返覆盇断言）。**第三步**加上宏与 regex 阶段：`macros.heads` 是「宏头 → 解析次数」（折叠小写、首次出现序）、`charsBefore`/`charsAfter` 是展开前后字符数（UTF-16 code unit），只在真的采到时才出现；`regex.applied` 是改写了这段文本的规则名（取脚本 `scriptName`，否则 id），conversation 聚合行带的是所有楼层命中规则的**并集**——prompt 方向的 regex 只跑在楼层上，所以那是它唯一能被如实记录的行。**第四步**加上预算的账：`PromptItemization.overflow?: PromptOverflow` 带 `droppedFloors`（裁掉的楼层数，与顶层 `droppedHistory` 同一个数）、`droppedTokens`（这些楼层的 token 重量）与可选的 `droppedIds`（`history.N`，供面板对齐对话）；这三个数都是**裁剪那一趟自己算的**，不是调用方重算一遍——两次求和迟早会对不上。一个自建 marker（如搜索扩展的 `搜索内容注入`）今天归属 `preset` 而不是 card：槽位是预设声明的，没有卡字段在它后面。字段全部可选，旧宿主的记录新前端能解析、新宿主的记录旧前端逐字段忽略。报告只带来源名、原因、宏头、规则名与数字，不带正文——正文留在宿主会话里（`LayoutPart.text` 不上契约）。
